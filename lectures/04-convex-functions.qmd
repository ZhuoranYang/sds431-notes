...
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ZhuoranYang/sds431-notes/blob/main/notebooks/03a-convex-functions.ipynb) **Convex Function Properties** --- visualizing epigraphs, level sets, and tangent-line characterizations
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ZhuoranYang/sds431-notes/blob/main/notebooks/04a-convex-functions-3d.ipynb) **3D Convex Geometry** --- interactive 3D plots of convex sets, ellipsoids, convex hulls, and level sets
:::
...
<div class="figure-container">
  ...
  <div class="figure-caption">Figure 4.1: A convex function in 3D: f(x₁,x₂) = x₁² + x₂². The surface lies below any chord, and every sublevel set is convex.</div>
</div>
...
### Strictly Convex and Strongly Convex Functions {#sec-strict-strong-convex}
...
A function $f : D \subseteq \mathbb{R}^n \to \mathbb{R}$ is **strictly convex** if $D$ is a convex set, and for all $x, y \in D$ with $x \neq y$ and all $\lambda \in (0,1)$:
...
::: {#def-strongly-convex}
## $m$-Strongly Convex Function

A function $f : D \subseteq \mathbb{R}^n \to \mathbb{R}$ is **$m$-strongly convex** (with parameter $m > 0$) if and only if the function $f(x) - \frac{m}{2}\|x\|_2^2$ is convex.
:::
...
A function $f$ is **concave** if $(-f)$ is convex; **strictly concave** if $(-f)$ is strictly convex; **$m$-strongly concave** if $(-f)$ is $m$-strongly convex. Concave functions are also defined on convex domains.

If $f$ is both convex and concave, then $f$ must be **affine**: $f(x) = a^\top x + b$.
:::

### Examples of Convex Functions {#sec-convex-function-examples}

::: {#exm-univariate-convex}
## Univariate Convex Functions ($f : \mathbb{R} \to \mathbb{R}$)

The following functions of a single variable are convex (verify by checking $f'' \geq 0$):
...
## Multivariate Convex Functions ($f : \mathbb{R}^n \to \mathbb{R}$)

**(i) Affine functions.** $f(x) = a^\top x + b$ is both convex and concave for any $a \in \mathbb{R}^n$, $b \in \mathbb{R}$.

::: {.proof}
$f(\lambda x + (1-\lambda)y) = a^\top(\lambda x + (1-\lambda)y) + b = \lambda(a^\top x + b) + (1-\lambda)(a^\top y + b) = \lambda \, f(x) + (1-\lambda) \, f(y)$. Equality holds, so $f$ is both convex and concave. $\blacksquare$
:::
...
::: {.proof}
$f(\lambda x + (1-\lambda)y) \leq f(\lambda x) + f((1-\lambda)y) = \lambda \, f(x) + (1-\lambda) \, f(y)$, using the triangle inequality and then homogeneity. $\blacksquare$
:::
...
::: {.proof}
**Proof via second-order condition.** Let $z_j = e^{x_j}$ and $\mathbf{1}_n^\top z = \sum_j z_j$. The gradient is $\nabla f(x) = \frac{1}{\mathbf{1}_n^\top z} \cdot z$ (the softmax vector). The Hessian is
...
![Common convex and concave functions in 1D, illustrating the chord property. (a--c) For convex functions, the chord between any two points lies above the graph (shaded gap). (d) For concave $\log x$, the chord lies below --- equivalently, $-\log x$ is convex.](figures/ch04-convex-functions-gallery.svg){#fig-convex-gallery}
...
::: {.proof}
**Proof of @thm-first-order-condition.** We prove both directions of the equivalence.
...
### Gradient Monotonicity {#sec-gradient-monotonicity}
...
Suppose $f$ is differentiable and $\text{dom}(f)$ is convex. Then $f$ is convex if and only if the gradient is **monotone**:
...
::: {.proof}
**Proof of @thm-gradient-monotonicity.**

**($\Longrightarrow$) Convexity implies gradient monotonicity.** If $f$ is convex, the first-order condition (@eq-first-order-convexity) gives two inequalities:
...
::: {#cor-strong-monotonicity}
## Strong Monotonicity of the Gradient

If $f$ is $m$-strongly convex and differentiable, then

$$\langle \nabla f(x) - \nabla f(y), \, x - y \rangle \geq m \, \|x - y\|_2^2 \quad \forall \, x, y \in \text{dom}(f).$$ {#eq-strong-monotonicity}

Conversely, if the gradient satisfies @eq-strong-monotonicity for some $m > 0$, then $f$ is $m$-strongly convex.
:::

::: {.proof}
Since $f$ is $m$-strongly convex, the function $\widetilde{f}(x) = f(x) - \frac{m}{2}\|x\|_2^2$ is convex. The first-order condition for $\widetilde{f}$ gives, for all $x, y$:
...
::: {.callout-note appearance="simple"}
**Comparison.** Gradient monotonicity (@thm-gradient-monotonicity) says $\langle \nabla f(x) - \nabla f(y), x - y \rangle \geq 0$; strong monotonicity (@cor-strong-monotonicity) upgrades this to $\geq m\|x - y\|_2^2$. The parameter $m$ measures the minimum curvature: larger $m$ means the gradients spread apart faster as $x$ and $y$ separate. Setting $m = 0$ recovers the ordinary monotonicity condition for convex (but not strongly convex) functions.
:::
...
::: {.proof}
**Proof of @thm-second-order-condition.** We prove both directions.

**($\Longrightarrow$) Convexity implies $\nabla^2 f(x) \succeq 0$.** Assume $f$ is convex. Fix any $x \in \text{dom}(f)$ and any direction $v \in \mathbb{R}^n$. Define $g(t) = f(x + tv)$, which is a convex function of $t$ (by restriction to a line, @thm-restriction-to-line). Since $g$ is a univariate convex function and is twice differentiable, $g''(0) \geq 0$. Computing:

$$g''(0) = v^\top \nabla^2 f(x) \, v \geq 0.$$

Since this holds for every $v \in \mathbb{R}^n$, the Hessian $\nabla^2 f(x)$ is positive semidefinite.

**($\Longleftarrow$) $\nabla^2 f(x) \succeq 0$ everywhere implies convexity.** Assume $\nabla^2 f(x) \succeq 0$ for all $x$. We show the first-order condition $f(y) \geq f(x) + \nabla f(x)^\top(y - x)$ holds for all $x, y$, which implies convexity by @thm-first-order-condition.
...
### Zero-th Order Characterization: Restriction to a Line {#sec-zeroth-order-char}
...
::: {#thm-restriction-to-line}
## Zero-th Order Characterization

A function $f : D \subseteq \mathbb{R}^n \to \mathbb{R}$ is (strictly) convex if and only if for every $u \in D$ and every direction $v \in \mathbb{R}^n$, the one-dimensional function
...
::: {.proof}
**Proof of @thm-restriction-to-line.**
...
::: {#exm-logdet-concave}
## Example: $\log\det(X)$ is Concave

Consider $f(X) = \log\det(X)$ on the domain $\mathbb{S}_{++}^n = \{X \in \mathbb{R}^{n \times n} : X \succ 0\}$ (positive definite matrices). To show that $f$ is concave, we restrict to an arbitrary line: for any $X \succ 0$ and any symmetric matrix $V$, define
...
::: {#exm-x2-over-y}
## Example: $f(x, y) = x^2/y$ is Convex

Consider $f(x, y) = x^2/y$ on the domain $\{(x, y) : y > 0\}$. The gradient is
...
::: {#exm-geometric-mean-concave}
## Example: Geometric Mean is Concave

The geometric mean $f(x) = \bigl(\prod_{j=1}^n x_j\bigr)^{1/n}$ is concave on $\mathbb{R}_{++}^n = \{x : x_j > 0\}$.
...
### Summary of Equivalent Characterizations {#sec-equivalence-summary}
...
::: {#thm-convexity-equivalences}
## Equivalent Characterizations of Convexity

The following are equivalent for a continuously differentiable $f$ on a convex domain:

**(a)** $f$ is convex (chord inequality, @eq-convex-function-def).

**(b)** $f(y) \geq f(x) + \langle \nabla f(x), \, y - x \rangle$ for all $x, y$ (first-order condition, @eq-first-order-convexity).

**(c)** $\langle \nabla f(x) - \nabla f(y), \, x - y \rangle \geq 0$ for all $x, y$ (gradient monotonicity, @thm-gradient-monotonicity).

If additionally $\nabla f$ is continuously differentiable (i.e., $f$ is $C^2$):

**(d)** $\nabla^2 f(x) \succeq 0$ for all $x \in \text{dom}(f)$ (Hessian PSD, @thm-second-order-condition).
:::

In each case, replacing $\leq$ with $<$ (and $\succeq$ with $\succ$) gives the corresponding characterization of **strict** convexity.

### Practical Methods for Establishing Convexity {#sec-practical-convexity}

In practice, there are three main strategies for showing that a function is convex:

1. **Verify the definition** (chord inequality), often simplified by **restricting to a line** (@thm-restriction-to-line). This reduces the problem to checking convexity of 1D functions.
...
#### Method 1: Restriction to a Line {.unnumbered}
...
::: {#exm-method1-logdet}
## Example: $\log\det(X)$ is Concave (via Restriction to a Line)

We already proved this in @exm-logdet-concave: for any $X \succ 0$ and symmetric $V$, the restriction $g(t) = \log\det(X + tV) = \text{const} + \sum_j \log(1 + t\lambda_j)$ satisfies $g''(t) = -\sum_j \lambda_j^2 / (1 + t\lambda_j)^2 \leq 0$. Since every 1D restriction is concave, $\log\det$ is concave on $\mathbb{S}_{++}^n$.
...
#### Method 2: Hessian Analysis {.unnumbered}
...
- If the Hessian factors as $\alpha \cdot vv^\top$ with $\alpha \geq 0$, it is rank-one PSD (@exm-x2-over-y).
...
::: {#exm-method2-least-squares}
## Example: Least Squares $f(x) = \|Ax - b\|_2^2$ is Convex

Let $A \in \mathbb{R}^{m \times n}$ and $b \in \mathbb{R}^m$. Expanding $f(x) = (Ax - b)^\top(Ax - b) = x^\top A^\top A x - 2b^\top A x + \|b\|_2^2$, the gradient and Hessian are:
...
::: {#exm-method2-negative-entropy}
## Example: Negative Entropy $f(x) = \sum_{j=1}^n x_j \log x_j$ is Convex

Let $f(x) = \sum_{j=1}^n x_j \log x_j$ on $\mathbb{R}_{++}^n = \{x : x_j > 0\}$. The gradient and Hessian are:
...
#### Method 3: Operations That Preserve Convexity {.unnumbered}
...
::: {#exm-method3-log-barrier}
## Example: Log-Barrier Function $f(x) = -\sum_{i=1}^m \log(b_i - a_i^\top x)$

The function $f(x) = -\sum_{i=1}^m \log(b_i - a_i^\top x)$ on the domain $\{x : a_i^\top x < b_i \;\forall\, i\}$ is convex. Here is the reasoning, purely from preservation rules:
...
::: {#exm-method3-norm-affine}
## Example: $f(x) = \|Ax + b\|$ for Any Norm

For any norm $\|\cdot\|$ on $\mathbb{R}^m$, any $A \in \mathbb{R}^{m \times n}$, and any $b \in \mathbb{R}^m$, the function $f(x) = \|Ax + b\|$ is convex on $\mathbb{R}^n$.

1. Any norm $g(z) = \|z\|$ is convex (proved in @exm-common-convex-concave-functions via triangle inequality + homogeneity).
...
The following six rules allow us to build convex functions from simpler ones. Together, they handle the vast majority of objectives encountered in practice.
...
```{=html}
<div class="figure-container">
  <div id="fig-func-operations"></div>
  <div class="figure-caption">Figure: Operations preserving convexity of functions. (a) Nonnegative weighted sum of two convex functions is convex. (b) Pointwise maximum of affine functions produces a convex piecewise-linear function. (c) Composition of convex nondecreasing $h$ with convex $g$ is convex. (d) Affine composition: $f(Ax+b)$ preserves convexity.</div>
</div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ========== (a) Nonneg weighted sum ==========
  const xa = [], f1a = [], f2a = [], ga = [];
  for (let i = 0; i < N; i++) {
    const t = -2.5 + 5 * i / (N - 1);
    xa.push(t);
    f1a.push(t * t);
    f2a.push((t - 1.5) * (t - 1.5));
    ga.push(0.6 * t * t + 0.4 * (t - 1.5) * (t - 1.5));
  }

  // ========== (b) Pointwise max ==========
  const xb = [], l1 = [], l2 = [], l3 = [], maxb = [];
  for (let i = 0; i < N; i++) {
    const t = -2 + 4 * i / (N - 1);
    xb.push(t);
    const v1 = 1.2 * t + 0.5, v2 = -0.8 * t + 1.5, v3 = 0.3 * t - 0.8;
    l1.push(v1); l2.push(v2); l3.push(v3);
    maxb.push(Math.max(v1, v2, v3));
  }
  ...
  for (let i = 0; i < N; i++) {
    const t = -2 + 4 * i / (N - 1);
    xc.push(t);
    const gval = Math.max(t, 0); // ReLU (convex)
    gc.push(gval);
    hgc.push(gval * gval); // t^2 is convex nondecreasing on t>=0
  }

  // ========== (d) Affine composition ==========
  const xd = [], fd_orig = [], fd_comp = [];
  for (let i = 0; i < N; i++) {
    const t = -2 + 4 * i / (N - 1);
    xd.push(t);
    fd_orig.push(t * t);
    fd_comp.push((2 * t - 1) * (2 * t - 1)); // f(2x-1) = (2x-1)^2
  }

  const traces = [
    // (a) Weighted sum
    {x: xa, y: f1a, mode: 'lines', line: {color: '#7b97ad', width: 1.5, dash: 'dot'},
     name: 'f\u2081(x)=x\u00b2', xaxis: 'x', yaxis: 'y', legendgroup: 'a'},
    {x: xa, y: f2a, mode: 'lines', line: {color: '#b8995e', width: 1.5, dash: 'dot'},
     name: 'f\u2082(x)=(x\u22121.5)\u00b2', xaxis: 'x', yaxis: 'y', legendgroup: 'a'},
    {x: xa, y: ga, mode: 'lines', line: {color: '#7a9a7e', width: 3},
     name: '0.6f\u2081+0.4f\u2082', xaxis: 'x', yaxis: 'y', legendgroup: 'a'},

    // (b) Pointwise max
    {x: xb, y: l1, mode: 'lines', line: {color: '#7b97ad', width: 1, dash: 'dot'},
     xaxis: 'x2', yaxis: 'y2', showlegend: false},
    {x: xb, y: l2, mode: 'lines', line: {color: '#b8995e', width: 1, dash: 'dot'},
     xaxis: 'x2', yaxis: 'y2', showlegend: false},
    {x: xb, y: l3, mode: 'lines', line: {color: '#907ea0', width: 1, dash: 'dot'},
     xaxis: 'x2', yaxis: 'y2', showlegend: false},
    {x: xb, y: maxb, mode: 'lines', line: {color: '#c47a6a', width: 3},
     name: 'max(f\u2081,f\u2082,f\u2083)', xaxis: 'x2', yaxis: 'y2'},

    // (c) Composition
    {x: xc, y: gc, mode: 'lines', line: {color: '#7b97ad', width: 1.5, dash: 'dot'},
     name: 'g(x)=ReLU(x)', xaxis: 'x3', yaxis: 'y3'},
    {x: xc, y: hgc, mode: 'lines', line: {color: '#7a9a7e', width: 3},
     name: 'h(g(x))=ReLU(x)\u00b2', xaxis: 'x3', yaxis: 'y3'},

    // (d) Affine composition
    {x: xd, y: fd_orig, mode: 'lines', line: {color: '#7b97ad', width: 1.5, dash: 'dot'},
     name: 'f(x)=x\u00b2', xaxis: 'x4', yaxis: 'y4'},
    {x: xd, y: fd_comp, mode: 'lines', line: {color: '#b8995e', width: 3},
     name: 'f(2x\u22121)=(2x\u22121)\u00b2', xaxis: 'x4', yaxis: 'y4'},
  ];

  const ann = (text, x, y, col) => ({
    text: '<b>'+text+'</b>', x, y, xref: 'paper', yref: 'paper',
    showarrow: false, font: {size: 13, color: col || '#3d3530'}
  });
  ...
  const layout = {
    ...
    xaxis:  {title: 'x', domain: [0, 0.44], zeroline: true, zerolinecolor: '#d4c9b8', gridcolor: '#e8e2d8'},
    yaxis:  {title: '', domain: [0.57, 1], range: [-0.5, 8], zeroline: false, gridcolor: '#e8e2d8'},
    xaxis2: {title: 'x', domain: [0.56, 1], zeroline: true, zerolinecolor: '#d4c9b8', gridcolor: '#e8e2d8'},
    yaxis2: {title: '', domain: [0.57, 1], range: [-2.5, 4], zeroline: false, gridcolor: '#e8e2d8'},
    xaxis3: {title: 'x', domain: [0, 0.44], zeroline: true, zerolinecolor: '#d4c9b8', gridcolor: '#e8e2d8'},
    yaxis3: {title: '', domain: [0, 0.43], range: [-0.5, 5], zeroline: false, gridcolor: '#e8e2d8'},
    xaxis4: {title: 'x', domain: [0.56, 1], zeroline: true, zerolinecolor: '#d4c9b8', gridcolor: '#e8e2d8'},
    yaxis4: {title: '', domain: [0, 0.43], range: [-1, 12], zeroline: false, gridcolor: '#e8e2d8'},
    margin: {t: 10, b: 40, l: 40, r: 10},
    showlegend: true,
    legend: {x: 0.5, y: -0.12, orientation: 'h', font: {size: 10},
             xanchor: 'center', yanchor: 'top'},
    annotations: [
      ann('(a) Nonneg. weighted sum', 0.22, 1.03, '#7a9a7e'),
      ann('(b) Pointwise maximum', 0.78, 1.03, '#c47a6a'),
      ann('(c) Composition h\u2218g', 0.22, 0.50, '#7a9a7e'),
      ann('(d) Affine composition', 0.78, 0.50, '#b8995e'),
    ],
  };
  Plotly.newPlot('fig-func-operations', traces, layout, cfg);
});
</script>
```

We now derive each rule. Rules 1--4 and 6 follow directly from the definition of convexity (@eq-convex-function-def); Rule 5 requires the chain rule and is the most instructive to work through carefully, especially in the multivariate setting.

::: {.proof}
**Proof of @thm-operations-convex-functions.** Throughout, $x, y \in \mathbb{R}^n$ are arbitrary points in the domain and $\lambda \in [0,1]$.
...
*Epigraph interpretation (alternative proof).* The epigraph of the pointwise max is $\text{epi}(\max_i f_i) = \{(x,t) : f_i(x) \leq t \;\;\forall\, i\} = \bigcap_{i=1}^k \text{epi}(f_i)$. Since each $\text{epi}(f_i)$ is convex (@thm-epigraph-convex), their intersection is convex (@thm-operations-convex-sets, Rule 1), so $\max_i f_i$ is convex.
...
### Additional Preservation Rules {#sec-additional-preservation}
...
::: {.proof}
**Proof (Vector composition, multivariate Hessian analysis).** Assume all functions are twice differentiable. Let $z = g(x) \in \mathbb{R}^k$ and write the Jacobian of $g$ as $J_g(x) \in \mathbb{R}^{k \times n}$, where the $i$-th row is $\nabla g_i(x)^\top$. By the multivariate chain rule:
...
::: {.callout-tip}
## Multivariate Composition Examples
...
### Examples of Preservation Rules {#sec-preservation-examples}

::: {#exm-preservation-applications}
## Applications of Preservation Rules

**(i) Piecewise linear functions.** $f(x) = \max_{i \in [m]}\{a_i^\top x + b_i\}$ is convex (Rule 3: max of affine functions). This is the most general form of a convex piecewise linear function.
...
## Epigraph and Connections to Convex Sets {#sec-epigraph}
...
The **epigraph** of a function $f : \mathbb{R}^n \to \mathbb{R}$ is the subset of $\mathbb{R}^{n+1}$ defined as
...
```{=html}
<div class="figure-container">
  <div id="fig-epigraph-2d"></div>
  <div class="figure-caption">Figure: Epigraphs of a convex function (left, convex set) and a non-convex function (right, non-convex set). The shaded region above each curve is the epigraph.</div>
</div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const N = 200;
  const x1 = [], y1 = [], x1_fill = [], y1_fill = [];
  for (let i = 0; i < N; i++) {
    const t = -2 + 4 * i / (N - 1);
    x1.push(t); y1.push(t * t);
  }
  // Fill region above curve
  for (let i = 0; i < N; i++) { x1_fill.push(x1[i]); y1_fill.push(y1[i]); }
  x1_fill.push(2); y1_fill.push(5);
  x1_fill.push(-2); y1_fill.push(5);

  // Non-convex function: f(x) = sin(2x) + 0.3x^2 + 1
  const x2 = [], y2 = [], x2_fill = [], y2_fill = [];
  for (let i = 0; i < N; i++) {
    const t = -2 + 4 * i / (N - 1);
    x2.push(t); y2.push(Math.sin(2*t) + 0.3*t*t + 1);
  }
  for (let i = 0; i < N; i++) { x2_fill.push(x2[i]); y2_fill.push(y2[i]); }
  x2_fill.push(2); y2_fill.push(5);
  x2_fill.push(-2); y2_fill.push(5);

  const traces = [
    {x: x1_fill, y: y1_fill, fill: 'toself', fillcolor: 'rgba(123,151,173,0.18)',
     line: {color: 'rgba(0,0,0,0)'}, showlegend: false, xaxis: 'x', yaxis: 'y'},
    {x: x1, y: y1, mode: 'lines', line: {color: '#7b97ad', width: 3},
     name: 'f(x) = x² (convex)', xaxis: 'x', yaxis: 'y'},

    {x: x2_fill, y: y2_fill, fill: 'toself', fillcolor: 'rgba(196,122,106,0.18)',
     line: {color: 'rgba(0,0,0,0)'}, showlegend: false, xaxis: 'x2', yaxis: 'y2'},
    {x: x2, y: y2, mode: 'lines', line: {color: '#c47a6a', width: 3},
     name: 'non-convex f', xaxis: 'x2', yaxis: 'y2'},
  ];

  const layout = {
    paper_bgcolor: 'rgba(0,0,0,0)', plot_bgcolor: 'rgba(0,0,0,0)',
    font: {family: '"Source Serif 4", Georgia, serif', size: 13},
    grid: {rows: 1, columns: 2, pattern: 'independent'},
    xaxis:  {title: 'x', domain: [0, 0.46], zeroline: false, gridcolor: '#e0d9d1'},
    yaxis:  {title: 't', range: [-0.5, 5], zeroline: false, gridcolor: '#e0d9d1'},
    xaxis2: {title: 'x', domain: [0.54, 1], zeroline: false, gridcolor: '#e0d9d1'},
    yaxis2: {title: 't', range: [-0.5, 5], zeroline: false, gridcolor: '#e0d9d1'},
    showlegend: true, legend: {x: 0.35, y: 1.0, font: {size: 12}},
    margin: {t: 30, b: 50, l: 50, r: 20},
    annotations: [
      {text: '<b>Convex epigraph</b>', x: 0.23, y: 1.08, xref: 'paper', yref: 'paper',
       showarrow: false, font: {size: 14, color: '#7b97ad'}},
      {text: '<b>Non-convex epigraph</b>', x: 0.77, y: 1.08, xref: 'paper', yref: 'paper',
       showarrow: false, font: {size: 14, color: '#c47a6a'}},
      {text: 'epi(f)', x: 0, y: 4, xref: 'x', yref: 'y',
       showarrow: false, font: {size: 15, color: '#7b97ad'}},
      {text: 'epi(f)', x: 0, y: 4.3, xref: 'x2', yref: 'y2',
       showarrow: false, font: {size: 15, color: '#c47a6a'}},
    ],
  };
  Plotly.newPlot('fig-epigraph-2d', traces, layout, plotlyConfig);
});
</script>
```
...
A function $f : D \subseteq \mathbb{R}^n \to \mathbb{R}$ is convex if and only if its epigraph $\text{epi}(f) \subseteq \mathbb{R}^{n+1}$ is a convex set.
:::
...
![Sublevel sets illustrated in one and two dimensions. (a) For $f(x) = \frac{1}{2}x^2$, each sublevel set $C_\alpha$ is a closed interval, and smaller $\alpha$ gives a smaller interval --- the sets are nested. (b) For $f(x_1, x_2) = x_1^2 + 2x_2^2$, the sublevel sets are nested ellipses shrinking toward the minimizer $x^*$.](figures/ch04-sublevel-sets.svg){#fig-sublevel-sets}
...
![Convexity versus quasiconvexity. (Left) A convex function always has convex sublevel sets. (Center) The function $f(x) = 3(1-e^{-x^2/2})$ is quasiconvex --- its sublevel sets are convex intervals --- but it is not convex, as shown by the chord lying below the curve. (Right) A non-quasiconvex function has disconnected sublevel sets at some level $\alpha$.](figures/ch04-quasiconvex.svg){#fig-quasiconvex}
...
Let $X$ be a random variable taking values in the domain of a convex function $f$. Then
...
::: {.proof}
**Proof of @thm-jensen.** We give two proofs: one for discrete random variables (by induction) and one for differentiable $f$ (via the first-order condition).
...
**Proof 2 (General case, via first-order condition).** Assume $f$ is differentiable. Let $\mu = \mathbb{E}[X]$. By the first-order condition (@eq-first-order-convexity) applied at $x = \mu$:
...
::: {#exm-jensen-variance}
## Example: Variance is Nonnegative

Take $f(u) = u^2$, which is convex. Jensen's inequality gives
...
## Optimality of Convex Functions {#sec-optimality-convex-functions}
...
::: {#thm-local-equals-global}
## Local Minimum $\iff$ Stationary Point $\iff$ Global Minimum

Let $f : \mathbb{R}^n \to \mathbb{R}$ be a convex and continuously differentiable function. Consider the unconstrained problem $\min_{x \in \mathbb{R}^n} f(x)$. Then the following are equivalent:
...
**(2) $\Rightarrow$ (3): Stationary point implies global minimum.** If $\nabla f(x^*) = 0$, then by the first-order condition for convex functions (@eq-first-order-convexity):
