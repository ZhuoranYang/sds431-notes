...
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ZhuoranYang/sds431-notes/blob/main/notebooks/14a-newton-method.ipynb) **Newton's Method** --- convergence analysis, failure modes, Newton fractals, and comparison with gradient descent
:::
...
![Newton's method jumps to the minimizer of the local quadratic approximation $\widehat{f}_x(y)$, using curvature information via the Hessian. The Newton direction takes a single step from the current iterate to the minimizer of the quadratic model.](figures/ch03-newton-step.svg){#fig-quadratic-approx}
...
![Newton--Raphson for root finding: at each iterate $x^k$, draw the tangent line to $F(x)$ and move to its $x$-intercept. The iterates converge rapidly to the root $x^* = 3$.](figures/ch03-newton-raphson-geometry.svg){#fig-newton-raphson}
...
![Affine invariance of Newton's method: applying a linear change of variables transforms the iterates but preserves the convergence trajectory. Gradient descent (left) slows down on ill-conditioned problems, while Newton's method (right) follows the same path regardless of the coordinate system.](figures/ch03-affine-invariance.svg){#fig-affine-invariance}
...
![The Armijo condition for backtracking line search. The step size $\alpha$ is acceptable when $f(x + \alpha d)$ (blue) lies below the Armijo line $f(x) + c \alpha \nabla f(x)^\top d$ (green dashed). The shaded region marks acceptable step sizes.](figures/ch14-armijo-condition.svg){#fig-armijo}
...
::: {#thm-two-phase}
## Two-Phase Convergence of Newton's Method
...
![Comparison of convergence rates on a log scale: sublinear convergence flattens, linear convergence is a straight line, and quadratic convergence (Newton) plummets as a double exponential --- the number of correct digits roughly doubles each iteration.](figures/ch03-convergence-rates.svg){#fig-convergence-comparison}
