...
### Necessary Conditions for Local Optimality {#sec-necessary-conditions}
...
#### First-Order Necessary Condition {#sec-first-order-necessary}

::: {#thm-first-order-necessary}
## First-Order Necessary Condition

Consider the unconstrained problem $\min_{x} f(x)$ where $f : \mathbb{R}^n \to \mathbb{R}$ is differentiable. If $x^*$ is a local minimizer, then
...
#### Second-Order Necessary Condition {#sec-second-order-necessary}
...
::: {#thm-second-order-necessary}
## Second-Order Necessary Condition

Consider the unconstrained problem $\min_{x} f(x)$ where $f : \mathbb{R}^n \to \mathbb{R}$ is twice differentiable. If $x^*$ is a local minimizer, then
...
::: {.proof}
From @thm-first-order-necessary, we already know $\nabla f(x^*) = 0$. It remains to show $\nabla^2 f(x^*) \succeq 0$.
...
A set $S$ is **bounded** if it fits inside some ball, i.e., there exists $r > 0$ such that $S \subseteq B(0, r)$. A set $S \subseteq \mathbb{R}^n$ is **compact** if it is both bounded and closed. Compactness has the following sequential characterization: if $\{a_n\}_{n \geq 1} \subseteq S$ is a sequence with a limit $a^* = \lim_{n \to \infty} a_n$, and $S$ is compact, then $a^* \in S$.
:::

![Left --- Interior point $p$ has an $\varepsilon$-ball fully contained in $S$; boundary point $q$ straddles inside and outside. Right --- Open sets exclude their boundary (dashed), closed sets include it (solid). A set is compact if and only if it is both closed and bounded.](figures/ch03-topology-concepts.svg){#fig-topology-concepts}
...
![Common convex sets in $\mathbb{R}^2$. Left --- a hyperplane $a^\top x = b$ divides the plane into two halfspaces. Center --- the $\ell_1$, $\ell_2$, and $\ell_\infty$ unit balls are all convex. Right --- a polyhedron is the intersection of finitely many halfspaces.](figures/ch03-convex-examples.svg){#fig-convex-examples}
...
::: {.proof}
**Proof of @thm-operations-convex-sets.** We prove each rule by taking two arbitrary points in the resulting set, forming their convex combination, and verifying the defining property. Throughout, all vectors lie in the appropriate $\mathbb{R}^n$ (or $\mathbb{R}^{n_1} \times \mathbb{R}^{n_2}$ for products) and $\lambda \in [0,1]$.
...
<div class="figure-container">
  <div id="fig-set-operations"></div>
  <div class="figure-caption">Figure: Operations on convex sets. (a) Intersection preserves convexity. (b) Union does NOT preserve convexity — the red segment leaves the set. (c) Affine image (rotation + scaling) of an ellipse remains convex. (d) Minkowski sum of a triangle and a disk produces a "rounded triangle," still convex.</div>
</div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Helper: generate ellipse points
  function ellipse(cx, cy, rx, ry, angle, N) {
    const xs = [], ys = [];
    const c = Math.cos(angle), s = Math.sin(angle);
    for (let i = 0; i <= N; i++) {
      const t = 2 * Math.PI * i / N;
      const ex = rx * Math.cos(t), ey = ry * Math.sin(t);
      xs.push(cx + c*ex - s*ey);
      ys.push(cy + s*ex + c*ey);
    }
    return {x: xs, y: ys};
  }
  ...
  // ========== (a) Intersection ==========
  const e1 = ellipse(0.3, 0, 1.2, 0.8, 0.3, 120);
  const e2 = ellipse(-0.3, 0, 1.2, 0.8, -0.3, 120);

  // Compute intersection region by sampling
  ...
  for (let i = 0; i <= 200; i++) {
    ...
    for (let r = 0; r <= 1.3; r += 0.01) {
      const px = r*Math.cos(a), py = r*Math.sin(a);
      if (inEllipse(px,py,0.3,0,1.2,0.8,0.3) && inEllipse(px,py,-0.3,0,1.2,0.8,-0.3)) {
        intX.push(px); intY.push(py); break;
      }
    }
  }

  // ========== (b) Union (NOT convex) ==========
  // Two circles with gap
  const c1 = ellipse(-1, 0, 0.7, 0.7, 0, 80);
  const c2 = ellipse(1, 0, 0.7, 0.7, 0, 80);

  // ========== (c) Affine image ==========
  const origE = ellipse(0, 0, 1, 0.5, 0, 100);
  // Apply A = [[1.2, 0.6],[0.3, 0.9]], b = [0, 0]
  const affX = [], affY = [];
  for (let i = 0; i < origE.x.length; i++) {
    affX.push(1.2*origE.x[i] + 0.6*origE.y[i]);
    affY.push(0.3*origE.x[i] + 0.9*origE.y[i]);
  }

  // ========== (d) Minkowski sum: triangle + disk ==========
  const tri = polygon(0, 0, 1.0, 3, Math.PI/2);
  // Minkowski sum with disk of radius 0.3
  const minkX = [], minkY = [];
  const rd = 0.3;
  for (let i = 0; i < tri.x.length - 1; i++) {
    const x1 = tri.x[i], y1 = tri.y[i];
    const x2 = tri.x[i+1], y2 = tri.y[i+1];
    // Edge direction
    const dx = x2-x1, dy = y2-y1;
    const len = Math.sqrt(dx*dx+dy*dy);
    const nx = -dy/len, ny = dx/len; // outward normal
    // Offset edge
    const ox1 = x1+rd*nx, oy1 = y1+rd*ny;
    const ox2 = x2+rd*nx, oy2 = y2+rd*ny;
    // Arc at vertex i+1
    const a1 = Math.atan2(ny, nx);
    const nx2_ = i+1 < tri.x.length-1 ? -(tri.y[i+2]-tri.y[i+1]) : -(tri.y[1]-tri.y[0]);
    const ny2_ = i+1 < tri.x.length-1 ? (tri.x[i+2]-tri.x[i+1]) : (tri.x[1]-tri.x[0]);
    const len2 = Math.sqrt(nx2_*nx2_+ny2_*ny2_);
    const a2 = Math.atan2(ny2_/len2, nx2_/len2);
    // Add edge points
    for (let t = 0; t <= 1; t += 0.05) {
      minkX.push(ox1 + t*(ox2-ox1));
      minkY.push(oy1 + t*(oy2-oy1));
    }
    // Add rounded corner
    ...
    for (let t = 0; t <= 1; t += 0.05) {
      ...
      minkX.push(x2 + rd*Math.cos(a));
      minkY.push(y2 + rd*Math.sin(a));
    }
  }

  const traces = [
    // (a) Intersection
    {x: e1.x, y: e1.y, mode: 'lines', line: {color: '#7b97ad', width: 2},
     fill: 'toself', fillcolor: 'rgba(123,151,173,0.12)', xaxis: 'x', yaxis: 'y',
     showlegend: false, hoverinfo: 'skip'},
    {x: e2.x, y: e2.y, mode: 'lines', line: {color: '#b8995e', width: 2},
     fill: 'toself', fillcolor: 'rgba(184,153,94,0.12)', xaxis: 'x', yaxis: 'y',
     showlegend: false, hoverinfo: 'skip'},
    {x: intX, y: intY, mode: 'markers', marker: {color: '#7a9a7e', size: 2.5},
     xaxis: 'x', yaxis: 'y', showlegend: false, hoverinfo: 'skip'},

    // (b) Union (NOT convex)
    {x: c1.x, y: c1.y, mode: 'lines', line: {color: '#7b97ad', width: 2},
     fill: 'toself', fillcolor: 'rgba(123,151,173,0.18)', xaxis: 'x2', yaxis: 'y2',
     showlegend: false, hoverinfo: 'skip'},
    {x: c2.x, y: c2.y, mode: 'lines', line: {color: '#b8995e', width: 2},
     fill: 'toself', fillcolor: 'rgba(184,153,94,0.18)', xaxis: 'x2', yaxis: 'y2',
     showlegend: false, hoverinfo: 'skip'},
    // Line segment that leaves the union
    {x: [-0.5, 0.5], y: [0.5, 0.5], mode: 'lines', line: {color: '#c47a6a', width: 3, dash: 'dash'},
     xaxis: 'x2', yaxis: 'y2', showlegend: false, hoverinfo: 'skip'},
    {x: [-0.5, 0.5], y: [0.5, 0.5], mode: 'markers', marker: {color: '#c47a6a', size: 7},
     xaxis: 'x2', yaxis: 'y2', showlegend: false, hoverinfo: 'skip'},

    // (c) Affine image
    {x: origE.x, y: origE.y, mode: 'lines', line: {color: '#7b97ad', width: 2, dash: 'dot'},
     fill: 'toself', fillcolor: 'rgba(123,151,173,0.10)', xaxis: 'x3', yaxis: 'y3',
     showlegend: false, hoverinfo: 'skip'},
    {x: affX, y: affY, mode: 'lines', line: {color: '#7a9a7e', width: 2.5},
     fill: 'toself', fillcolor: 'rgba(122,154,126,0.18)', xaxis: 'x3', yaxis: 'y3',
     showlegend: false, hoverinfo: 'skip'},

    // (d) Minkowski sum
    {x: tri.x, y: tri.y, mode: 'lines', line: {color: '#7b97ad', width: 2, dash: 'dot'},
     xaxis: 'x4', yaxis: 'y4', showlegend: false, hoverinfo: 'skip'},
    {x: minkX, y: minkY, mode: 'lines', line: {color: '#7a9a7e', width: 2.5},
     fill: 'toself', fillcolor: 'rgba(122,154,126,0.15)', xaxis: 'x4', yaxis: 'y4',
     showlegend: false, hoverinfo: 'skip'},
  ];
  ...
  const layout = {
    ...
    xaxis:  {domain:[0,0.44], zeroline:false, showgrid:false, showticklabels:false, scaleanchor:'y'},
    yaxis:  {domain:[0.55,1], zeroline:false, showgrid:false, showticklabels:false},
    xaxis2: {domain:[0.56,1], zeroline:false, showgrid:false, showticklabels:false, scaleanchor:'y2'},
    yaxis2: {domain:[0.55,1], zeroline:false, showgrid:false, showticklabels:false},
    xaxis3: {domain:[0,0.44], zeroline:false, showgrid:false, showticklabels:false, scaleanchor:'y3'},
    yaxis3: {domain:[0,0.45], zeroline:false, showgrid:false, showticklabels:false},
    xaxis4: {domain:[0.56,1], zeroline:false, showgrid:false, showticklabels:false, scaleanchor:'y4'},
    yaxis4: {domain:[0,0.45], zeroline:false, showgrid:false, showticklabels:false},
    margin: {t:10, b:10, l:10, r:10},
    annotations: [
      ann('(a) Intersection (convex)', 0.22, 1.02, '#7a9a7e'),
      ann('(b) Union (NOT convex)', 0.78, 1.02, '#c47a6a'),
      ann('(c) Affine image (convex)', 0.22, 0.50, '#7a9a7e'),
      ann('(d) Minkowski sum (convex)', 0.78, 0.50, '#7a9a7e'),
      {text:'C₁∩C₂', x:0, y:0, xref:'x', yref:'y', showarrow:false, font:{size:14, color:'#7a9a7e'}},
      {text:'C₁∪C₂', x:0, y:0.8, xref:'x2', yref:'y2', showarrow:false, font:{size:13, color:'#7b7067'}},
      {text:'✗', x:0, y:0.5, xref:'x2', yref:'y2', showarrow:false, font:{size:18, color:'#c47a6a'}},
      {text:'C', x:-0.2, y:-0.15, xref:'x3', yref:'y3', showarrow:false, font:{size:13, color:'#7b97ad'}},
      {text:'A·C+b', x:0.7, y:0.5, xref:'x3', yref:'y3', showarrow:false, font:{size:13, color:'#7a9a7e'}},
      {text:'C⊕B(0,r)', x:0, y:-0.75, xref:'x4', yref:'y4', showarrow:false, font:{size:13, color:'#7a9a7e'}},
    ],
  };
  Plotly.newPlot('fig-set-operations', traces, layout, cfg);
});
</script>
```
...
Let $C, D \subseteq \mathbb{R}^n$ be two convex sets with $C \cap D = \emptyset$. Then there exists $a \in \mathbb{R}^n$ ($a \neq 0$) and $b \in \mathbb{R}$ such that
...
::: {.callout-warning}
## Strict Separation Is Not Always Possible
...
::: {#thm-strict-separation}
## Strict Separating Hyperplane Theorem

Let $C, D \subseteq \mathbb{R}^n$ be two **closed** convex sets with at least one of them **bounded** (i.e., compact). If $C \cap D = \emptyset$, then there exists $a \in \mathbb{R}^n$ ($a \neq 0$) and $b \in \mathbb{R}$ such that
...
::: {.proof}
**Proof of @thm-strict-separation.**
