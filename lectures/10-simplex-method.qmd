...
Recall from the [previous chapter](09-lp-formulation-geometry.qmd) that a linear program in **canonical form** is:
...
## Deriving the Simplex Method {#sec-derivation}
...
$$\min \; z = c_B^\top x_B + c_N^\top x_N \quad \text{s.t.} \quad A_B x_B + A_N x_N = b, \;\; x_B \geq 0, \; x_N \geq 0.$$ {#eq-simplex-lp}

### Starting from a BFS {#sec-starting-bfs}
...
Substituting @eq-basic-from-nonbasic into the objective of @eq-simplex-lp:
...
$$z = \underbrace{c_B^\top A_B^{-1} b}_{z^* \text{ (current value)}} + r_N^\top x_N.$$ {#eq-objective-reduced}

Since $x_N = 0$ at the current BFS, the current objective value is $z^* = c_B^\top A_B^{-1}b$.

### Optimality Condition {#sec-optimality}

From @eq-objective-reduced, we can read off the optimality condition immediately.
...
### The Pivot Mechanism {#sec-pivot-mechanism}

This is the heart of the simplex method. The reduced cost equation @eq-objective-reduced tells us $z = z^* + r_N^\top x_N$, and we want to decrease $z$. Currently all non-basic variables are zero. The entire strategy reduces to two questions.

#### Question 1: Which non-basic variable should we increase? {.unnumbered}
...
If no such $q$ exists -- that is, $r_N \geq 0$ -- then increasing *any* non-basic variable can only increase or maintain the objective. **We are already at the optimum.** This is the optimality condition from @sec-optimality.

#### Question 2: How far can we increase $x_q$? {.unnumbered}

We want to push $x_q$ as large as possible (to decrease $z$ as much as possible), but we must maintain feasibility: all basic variables must stay nonneg. Setting $x_q = t > 0$ and keeping all other non-basic variables at zero, @eq-basic-from-nonbasic gives:
...
7. **Pivot:** Update $B \leftarrow B \cup \{q\} \setminus \{p\}$, $N \leftarrow N \cup \{p\} \setminus \{q\}$. Recompute $x_B^* = A_B^{-1}b$.
:::
...
### Why the Simplex Method Works {#sec-why-simplex}
...
2. **Optimality certificate.** When $r_N \geq 0$, the current BFS is provably optimal: @eq-objective-reduced shows that $z = z^* + r_N^\top x_N \geq z^*$ for all feasible $x_N \geq 0$.
...
## Simplex Tableau Quantities

Given a basis $B$ with BFS $x_B^* = A_B^{-1} b \geq 0$, define:

| Symbol | Definition | Meaning | Role in derivation |
|--------|-----------|---------|-------------------|
| $\widetilde{b}$ | $A_B^{-1} b$ | Current BFS values | $x_B^*$ |
| $\widetilde{A}$ | $A_B^{-1} A_N$ | How basic variables respond to non-basic ones | Exchange coefficients in @eq-basic-from-nonbasic |
| $\widetilde{c}$ | $c_N - \widetilde{A}^\top c_B$ | Cost of increasing each non-basic variable | Reduced costs $r_N$ from @def-reduced-cost |
| $\widetilde{\zeta}$ | $c_B^\top \widetilde{b}$ | Current objective value | $z^*$ from @eq-objective-reduced |
:::
...
$$\begin{pmatrix} \zeta \\ x_B \end{pmatrix} = \begin{pmatrix} \widetilde{\zeta} & \widetilde{c}^\top \\ \widetilde{b} & -\widetilde{A} \end{pmatrix} \begin{pmatrix} 1 \\ x_N \end{pmatrix}.$$ {#eq-tableau-form}
...
### Pivoting in Tableau Notation {#sec-pivoting}

The pivot mechanism from @sec-pivot-mechanism translates directly into tableau language. In terms of the tableau quantities:
...
::: {#def-ratio-test}
## Ratio Test (Tableau Form)

$$t^* = \min\left\{\frac{\widetilde{b}_i}{\widetilde{A}_{iq}} : \widetilde{A}_{iq} > 0, \;\; i \in B\right\}, \qquad p = \arg\min_{i \in B}\left\{\frac{\widetilde{b}_i}{\widetilde{A}_{iq}} : \widetilde{A}_{iq} > 0\right\}.$$ {#eq-ratio-test}
:::
...
3. **Degeneracy warning:** If $\widetilde{b}_p = 0$, then $t^* = 0$ and the objective does not decrease. The basis changes, but the BFS point stays put. This is **degeneracy** -- we return to it in @sec-degeneracy.
:::

## The Full Simplex Algorithm (Tableau Form) {#sec-full-algorithm}

The simplex method from @sec-simplex-algorithm can now be expressed concisely as a five-step loop over the tableau:
...
![Flowchart of the simplex method. The algorithm iterates through tableau computation, optimality check, entering variable selection, ratio test, and pivoting.](figures/simplex-flowchart.svg){#fig-simplex-flowchart width="85%"}
...
![The simplex method traces a path along edges of the feasible polytope. Starting from the origin (BFS $e$), it pivots to BFS $d$, then to the optimal BFS $a$ at $(0.5, 1.5)$ with $\zeta^* = -1.5$.](figures/simplex-path.svg){#fig-simplex-path}

![The simplex tableau at each iteration. The method starts with basis $B=\{3,4\}$ (slack variables), pivots twice, and terminates at the optimal basis $B=\{1,2\}$ with $x^* = (\frac{1}{2}, \frac{3}{2})$ and $f^* = -\frac{3}{2}$. Red entries indicate negative reduced costs that trigger a pivot; green entries confirm optimality.](figures/simplex-tableau.svg){#fig-simplex-tableau width="85%"}

```{=html}
<!-- Remove obsolete interactive tableau -->
<div style="display:none;">
<script>
(function() {
  var iterations = [
    {title: 'Iteration 0: B={3,4}, N={1,2}', B: [3,4], N: [1,2],
     b_tilde: [1,2], A_tilde: [[-1,1],[1,1]], c_tilde: [0,-1], zeta: 0,
     entering: 'q=2 (c\u0303\u2082=\u22121<0)', leaving: 'p=3 (min{1/1, 2/1}=1)',
     status: 'Not optimal: c\u0303\u2082 = \u22121 < 0'},
    {title: 'Iteration 1: B={2,4}, N={1,3}', B: [2,4], N: [1,3],
     b_tilde: [1,1], A_tilde: [[-1,1],[2,-1]], c_tilde: [-1,1], zeta: -1,
     entering: 'q=1 (c\u0303\u2081=\u22121<0)', leaving: 'p=4 (min{\u2014, 1/2}=0.5)',
     status: 'Not optimal: c\u0303\u2081 = \u22121 < 0'},
    {title: 'Iteration 2: B={1,2}, N={3,4}', B: [1,2], N: [3,4],
     b_tilde: [0.5,1.5], A_tilde: [[0.5,-0.5],[0.5,0.5]], c_tilde: [0.5,0.5], zeta: -1.5,
  ...
  for (var k = 0; k < iterations.length; k++) {
    var it = iterations[k];
    var header = ['', 'x<sub>' + it.N[0] + '</sub>', 'x<sub>' + it.N[1] + '</sub>'];
    var col0 = ['\u03b6\u0303 = ' + it.zeta];
    var col1 = [fmt(it.c_tilde[0])];
    var col2 = [fmt(it.c_tilde[1])];
    var fc0 = ['rgba(184,153,94,0.15)'];
    var fc1 = ['rgba(184,153,94,0.15)'];
    var fc2 = ['rgba(184,153,94,0.15)'];
    for (var r = 0; r < it.B.length; r++) {
      col0.push('x<sub>' + it.B[r] + '</sub> = ' + it.b_tilde[r].toFixed(1));
      col1.push(fmt(-it.A_tilde[r][0]));
      col2.push(fmt(-it.A_tilde[r][1]));
      fc0.push('rgba(123,143,163,0.08)');
      fc1.push('rgba(123,143,163,0.08)');
      fc2.push('rgba(123,143,163,0.08)');
    }
    allTraces.push({
      type: 'table',
      header: {values: header, fill: {color: 'rgba(123,143,163,0.25)'},
        font: {size: 13}, align: 'center', height: 30},
      cells: {values: [col0, col1, col2], fill: {color: [fc0, fc1, fc2]},
        font: {size: 13}, align: 'center', height: 28},
      visible: (k === 0)
    });
  }
  ...
  var buttons = [];
  for (var k = 0; k < iterations.length; k++) {
    var vis = [false, false, false]; vis[k] = true;
    var it = iterations[k];
    var statusText = '<b>' + it.status + '</b>';
    if (it.entering !== 'None') {
      statusText += '<br>Entering: ' + it.entering + '<br>Leaving: ' + it.leaving;
    }
    buttons.push({label: it.title, method: 'update',
      args: [{visible: vis},
        {annotations: [{x: 0.5, y: -0.15, xref: 'paper', yref: 'paper',
          text: statusText, showarrow: false, font: {size: 12}, align: 'center'}]}]
    });
  }

  var initStatus = '<b>' + iterations[0].status + '</b><br>Entering: ' + iterations[0].entering + '<br>Leaving: ' + iterations[0].leaving;

  var layout = themeLayout({
    updatemenus: [{type: 'dropdown', direction: 'down',
      x: 0.5, xanchor: 'center', y: 1.15, yanchor: 'top',
      buttons: buttons, font: {size: 12}}],
    annotations: [{x: 0.5, y: -0.15, xref: 'paper', yref: 'paper',
      text: initStatus, showarrow: false, font: {size: 12}, align: 'center'}],
    margin: {t: 60, b: 80, l: 20, r: 20}, height: 280
  });

  Plotly.newPlot('fig-tableau-iterations', allTraces, layout, plotlyConfig);
})();
</script>
</div>
```
...
The simplex method requires a starting BFS. When $b \geq 0$, this is easy: use the slack-variable basis $B = \{n+1, \ldots, n+m\}$, which gives $x_B^* = b \geq 0$ (as we noted in @sec-starting-bfs). But what if some $b_i < 0$? Then $x_B^* = b$ has negative entries, so the slack-variable basis is *infeasible*. We cannot simply pick a basis and hope it works -- we need a systematic method to either find a feasible BFS or certify that none exists.

### The Key Idea {#sec-aux-idea}
...
### The Initialization Algorithm {#sec-init-algorithm}
...
```{=html}
<div id="fig-simplex-interactive" style="width:100%;max-width:700px;margin:auto;"></div>
<p class="figure-caption" style="text-align:center;margin-top:0.5em;">Figure 10.4: Interactive simplex method demo. Use the slider to step through pivot iterations. The red arrow shows the direction of movement, and the highlighted vertex is the current BFS.</p>
<script>
(function() {
  var poly_x = [0, 2, 0.5, 0, 0];
  var poly_y = [0, 0, 1.5, 1, 0];
  ...
  var xx = Array.from({length: 50}, function(_, i) { return -0.5 + 3.5 * i / 49; });
  ...
  for (var step = 0; step < path.length; step++) {
    ...
    // Feasible region
    frameData.push({x: poly_x, y: poly_y, fill: 'toself',
      fillcolor: 'rgba(123,143,163,0.15)', line: {color: '#7b97ad', width: 2.5},
      name: 'Feasible region', mode: 'lines'});

    // Constraint lines
    frameData.push({x: xx, y: xx.map(function(v){return v+1;}), mode: 'lines',
      line: {color: '#968a80', dash: 'dot', width: 1}, showlegend: false, hoverinfo: 'skip'});
    frameData.push({x: xx, y: xx.map(function(v){return 2-v;}), mode: 'lines',
      line: {color: '#968a80', dash: 'dot', width: 1}, showlegend: false, hoverinfo: 'skip'});

    // Other BFS (gray)
    frameData.push({x: [2], y: [0], mode: 'markers+text',
      marker: {color: '#aaa', size: 7}, text: ['b'],
      textposition: 'bottom right', textfont: {size: 10, color: '#aaa'},
      showlegend: false, hoverinfo: 'skip'});

    // Visited path so far
    var px = [], py = [];
    for (var k = 0; k <= step; k++) { px.push(path[k].x); py.push(path[k].y); }
    frameData.push({x: px, y: py, mode: 'lines',
      line: {color: '#b56b6b', width: 3}, name: 'Simplex path', showlegend: false});

    // Visited vertices
    for (var k = 0; k <= step; k++) {
      var p = path[k];
      var isCurrent = (k === step);
      var sz = isCurrent ? 14 : 8;
      var sym = (isCurrent && step === path.length-1) ? 'star' : (isCurrent ? 'diamond' : 'circle');
      var col = isCurrent ? '#b56b6b' : '#8a6e5e';
      frameData.push({x: [p.x], y: [p.y], mode: 'markers+text',
        marker: {color: col, size: sz, symbol: sym, line: {width: 1.5, color: 'white'}},
        text: [p.label], textposition: 'top left',
        textfont: {size: 12, color: col},
        hovertext: ['BFS ' + p.label + ': (' + p.x + ',' + p.y + ')<br>B=' + p.B + ', \u03b6=' + p.obj],
    ...
    if (step < path.length - 1) {
      anns.push({x: path[step+1].x, y: path[step+1].y,
        ax: path[step].x, ay: path[step].y,
        xref: 'x', yref: 'y', axref: 'x', ayref: 'y',
        showarrow: true, arrowhead: 3, arrowsize: 1.5, arrowcolor: '#b56b6b', arrowwidth: 2});
    }
    anns.push({x: 0.02, y: 0.02, xref: 'paper', yref: 'paper',
      text: '<b>Step ' + step + ': B=' + path[step].B + ', \u03b6=' + path[step].obj + '</b><br>' + path[step].desc,
      showarrow: false, font: {size: 11}, align: 'left',
      bgcolor: 'rgba(255,255,255,0.85)', bordercolor: '#7b97ad', borderwidth: 1});

    frames.push({data: frameData, name: String(step), layout: {annotations: anns}});
  }

  var sliderSteps = [];
  for (var k = 0; k < frames.length; k++) {
    sliderSteps.push({args: [[String(k)], {mode: 'immediate', frame: {duration: 0, redraw: true}}],
      label: 'Step ' + k, method: 'animate'});
  }

  var layout = themeLayout({
    xaxis: themeAxis('x\u2081', {range: [-0.5, 2.8], zeroline: true, showline: true}),
    yaxis: themeAxis('x\u2082', {range: [-0.5, 2.3], zeroline: true, scaleanchor: 'x', showline: true}),
    showlegend: false,
    annotations: frames[0].layout.annotations,
    margin: {t: 20, b: 70, l: 55, r: 20},
    sliders: [{active: 0, pad: {t: 40}, steps: sliderSteps,
      currentvalue: {prefix: '', visible: true}, x: 0.1, len: 0.8}]
  });

  Plotly.newPlot('fig-simplex-interactive', frames[0].data, layout, plotlyConfig)
    .then(function() { return Plotly.addFrames('fig-simplex-interactive', frames); });
})();
</script>
```
