---
title: "SGD, Proximal Gradient, and Frank-Wolfe"
---
...
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ZhuoranYang/sds431-notes/blob/main/notebooks/08a-subgradient-proximal-gradient.ipynb) **Proximal Gradient** --- solving the Lasso with proximal operators
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ZhuoranYang/sds431-notes/blob/main/notebooks/08b-projected-gradient-frank-wolfe.ipynb) **Projected Gradient and Frank-Wolfe** --- constrained optimization on $\ell_1$-balls with signal recovery
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ZhuoranYang/sds431-notes/blob/main/notebooks/08c-tv-denoising.ipynb) **TV Denoising** --- total variation image denoising via proximal gradient in PyTorch
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ZhuoranYang/sds431-notes/blob/main/notebooks/08d-coordinate-descent.ipynb) **Coordinate Descent** --- coordinate descent methods and comparisons
:::
...
![SGD noise and convergence. Full gradient descent (green) converges smoothly to $x^*$. SGD with batch size $b = 1$ (terracotta) oscillates in a noise ball proportional to $\sigma^2/b$. Mini-batch SGD (blue) reduces noise and interpolates between the two.](figures/ch08-sgd-noise.svg){#fig-sgd-noise}
...
The Lasso is the prototypical example of a nonsmooth optimization problem in statistics:
...
**Composite objective functions** have the form
...
**Lasso:** $g(\theta) = \|y - A\theta\|_2^2$ (smooth), $\;h(\theta) = \lambda\|\theta\|_1$ (nonsmooth but simple).
...
### From GD to Proximal Gradient: The Key Idea {#proximal-idea}
...
The **proximal mapping** (or **proximal operator**) induced by a function $h$ with parameter $\alpha > 0$ is:
...
@fig-proximal-operator illustrates this trade-off geometrically for $h(z) = |z|$: the proximal operator finds the point that minimizes the sum of $h$ and the quadratic penalty centered at $x_k$.

![Geometric picture of the proximal operator for $h(z) = |z|$. The proximal step minimizes the sum of $h(z)$ and a quadratic penalty centered at the current iterate $x_k$, yielding the new iterate $x_{k+1}$.](figures/ch08-proximal-operator.svg){#fig-proximal-operator}
...
Let $F(x) = g(x) + h(x)$ where $g$ is $M$-smooth and $h$ is convex. With step size $\alpha = 1/M$, the proximal gradient method satisfies:

**Case 1 (Convex $g$).** Sublinear rate on function values:
$$F(x^k) - F(x^*) \leq \frac{M}{2k}\|x^0 - x^*\|_2^2.$$ {#eq-proximal-convergence}
...
::: {.callout-tip}
## Key Insight: Smoothness for Free

Comparing with the gradient descent convergence rates ([Chapter 6](06-gradient-descent.qmd)):

| Setting | GD on smooth $f$ | Proximal GD on $g + h$ |
|---|---|---|
| Convex | $O(1/k)$ | $O(1/k)$ |
...
The convergence guarantee above is only useful in practice if the proximal mapping (@eq-proximal-mapping) can be computed efficiently. After all, the proximal mapping itself is an optimization problem --- if it were as hard as the original problem, we would have gained nothing.
...
## Projection as Proximal Mapping
...
<div class="figure-container">
  ...
  <div class="figure-caption">Frank-Wolfe iteration: linearize f at x^k, minimize the linear surrogate over C to get y^k, then take a convex combination step.</div>
</div>
```
...
The Frank-Wolfe method converges at a rate of $O(1/K)$ (@eq-fw-convergence). The key advantage over projected gradient descent is that Frank-Wolfe only requires solving a linear optimization problem over $\mathcal{C}$ at each step (rather than a projection), which can be much cheaper for structured constraint sets such as polytopes or nuclear norm balls.
...
<style>
  ...
  .interactive-figure-container .slider-row label {
    font-size: 0.95em; color: #3d3530; min-width: 110px;
  }
...
<div class="interactive-figure-container" id="fwpgd-wrapper">
  ...
  <p class="fig-subtitle">
    min &frac12;&Vert;x &minus; p&Vert;&sup2; &ensp; s.t. &Vert;x&Vert;<sub>1</sub> &le; 1
    &emsp;|&emsp; <span style="color:#c47a6a">&starf;</span> = target p (outside the ball)
    &emsp;|&emsp; 30 iterations from origin
  </p>
  <div class="slider-row">
    <label for="fwpgd-angle">Target angle =</label>
    <input type="range" id="fwpgd-angle" min="5" max="85" step="1" value="55">
    <span class="slider-value" id="fwpgd-angle-val">55&deg;</span>
  </div>
  <div class="panels">
...
(function() {
  ...
  function makeLayout(title) {
    var axStyle = {gridcolor: C.grid, zerolinecolor: C.axis, linecolor: C.axis,
                   tickfont: {size: 12, family: font.family, color: font.color},
                   titlefont: {size: 14, family: font.family, color: font.color}};
    return {
      paper_bgcolor: C.bg, plot_bgcolor: C.plotBg, font: font, height: 430,
      margin: {t: 38, b: 50, l: 50, r: 16},
      title: {text: title, font: {size: 14, family: font.family, color: font.color}},
      xaxis: Object.assign({}, axStyle, {range: [-1.6, 1.8], title: 'x\u2081'}),
      yaxis: Object.assign({}, axStyle, {range: [-1.6, 1.8], title: 'x\u2082', scaleanchor: 'x'}),
  ...
  function update(angleDeg) {
    ...
    var p = [1.5*Math.cos(rad), 1.5*Math.sin(rad)];
    var pstar = projL1(p);
    ...
    var commonTraces = function(pathX, pathY, col, name) {
      return [
        {x: ballX, y: ballY, mode: 'lines', fill: 'toself',
         fillcolor: 'rgba(122,154,126,0.10)', line: {color: C.sage, width: 2},
         hoverinfo: 'skip'},
        {x: pathX, y: pathY, mode: 'lines+markers',
         line: {color: col, width: 2.5}, marker: {size: 5, color: col}, name: name},
        {x: [p[0]], y: [p[1]], mode: 'markers+text',
         ...
         textfont: {size: 12, family: font.family, color: C.terracotta}, showlegend: false},
        {x: [pstar[0]], y: [pstar[1]], mode: 'markers+text',
         marker: {size: 9, color: C.sage, symbol: 'diamond', line: {width: 1.5, color: 'white'}},
         text: ['x*'], textposition: 'bottom left',
         textfont: {size: 12, family: font.family, color: C.sage}, showlegend: false},
        {x: [0], y: [0], mode: 'markers',
         marker: {size: 7, color: C.axis, symbol: 'circle'}, showlegend: false}
      ];
    };
    ...
    var info = 'Target p = (' + p[0].toFixed(2) + ', ' + p[1].toFixed(2) + ')'
             + ' &ensp;|&ensp; Optimum x* = (' + pstar[0].toFixed(2) + ', ' + pstar[1].toFixed(2) + ')'
             + ' &ensp;|&ensp; <span style="color:' + C.blue + '"><b>PGD</b>: f = ' + fPGD.toExponential(2) + '</span>'
             + ' &ensp; <span style="color:' + C.gold + '"><b>FW</b>: f = ' + fFW.toExponential(2) + '</span>';
    document.getElementById('fwpgd-info').innerHTML = info;

    Plotly.react('fwpgd-left', commonTraces(pgdX, pgdY, C.blue, 'PGD'), makeLayout('Projected Gradient Descent'), plotCfg);
    Plotly.react('fwpgd-right', commonTraces(fwX, fwY, C.gold, 'Frank-Wolfe'), makeLayout('Frank-Wolfe'), plotCfg);
  }
