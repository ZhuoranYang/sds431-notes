...
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ZhuoranYang/sds431-notes/blob/main/notebooks/02a-automatic-differentiation.ipynb) **Automatic Differentiation** --- computing gradients and Hessians with autograd, PyTorch, and JAX
:::
...
<div class="figure-container">
  ...
  <div class="figure-caption">Unit norm balls in &#x211D;<sup>2</sup> for &#x2113;<sub>1</sub>, &#x2113;<sub>2</sub>, and &#x2113;<sub>&infin;</sub> norms. The set {x &isin; &#x211D;<sup>2</sup> : &#x2016;x&#x2016; &le; 1} for each norm.</div>
</div>
<script>
...
<div class="figure-container">
  ...
  <div class="figure-caption">The unit &#x2113;<sub>2</sub>-ball (left) mapped by A = [[1, 2], [0, 2]] to an ellipse (right). The operator norm &#x2016;A&#x2016;<sub>2</sub> equals the largest semi-axis length.</div>
</div>
<script>
document.addEventListener('DOMContentLoaded', () => {
  const plotlyConfig = {responsive: true, displayModeBar: false};
...
## Closed-Form Expressions for Induced Norms
...
::: {.callout-tip}
## Remark: Best Low-Rank Approximation (Eckart--Young Theorem)
...
<div class="figure-container">
  ...
  <div class="figure-caption">Eigenvalue spectrum of a symmetric matrix. The eigenvalues determine the shape of the associated quadratic form x<sup>&#x22A4;</sup>Ax.</div>
</div>
<script>
document.addEventListener('DOMContentLoaded', () => {
  const plotlyConfig = {responsive: true, displayModeBar: false};
...
## Linear Approximation via Gradient
...
## Quadratic Approximation via Hessian
...
#### Computational Graphs {#sec-computation-graphs}
...
![Computation graph for $f(x_1, x_2) = \sin(x_1) + x_1 \cdot x_2$. Inputs (blue) feed into intermediate nodes (sage) via elementary operations, which combine into the output (gold).](figures/ch02-computation-graph.svg){#fig-computation-graph}

#### A Worked Example {#sec-autodiff-example}
...
: Forward evaluation trace {.striped}
...
: Local partial derivatives (edge weights) {.striped}
...
#### Forward Mode AD {#sec-forward-mode}
...
: Forward mode trace for $\partial f / \partial x_1$ {.striped}
...
#### Reverse Mode AD (Backpropagation) {#sec-reverse-mode}
...
: Reverse mode trace (backpropagation) {.striped}
...
#### Forward vs. Reverse: When to Use Which {#sec-forward-vs-reverse}

![Forward mode (left, sage) propagates tangent derivatives from inputs to output. Reverse mode (right, rosewood) propagates adjoint derivatives from output to inputs. The same computation graph is traversed in opposite directions.](figures/ch02-forward-vs-reverse.svg){#fig-forward-vs-reverse}
...
![Backpropagation through a neural network. Forward arrows (solid, green) propagate activations from input $x$ through layers $W_1$, ReLU, $W_2$ to the loss $\mathcal{L}$. Backward arrows (dashed, red) propagate gradients via the chain rule: $\partial \mathcal{L}/\partial W_1 = (\partial \mathcal{L}/\partial W_2)(\partial W_2 / \partial \text{ReLU})(\partial \text{ReLU} / \partial W_1)$.](figures/ch03-backpropagation.svg){#fig-backpropagation}
