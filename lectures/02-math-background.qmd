...
## Norms: Vector and Matrix {#sec-norms}
...
A **vector norm** $\|\cdot\| : \mathbb{R}^n \to \mathbb{R}$ is a function that maps $x \in \mathbb{R}^n$ to $\|x\| \in \mathbb{R}$ satisfying the following properties:
...
### $\ell_p$-Norms and Norm Balls {#sec-lp-norms}

::: {#exm-lp-norms}
## Examples: $\ell_p$-Norms
...
<div class="figure-container">
  <div id="fig-norm-balls"></div>
  <div class="figure-caption">Figure 2.1: Unit norm balls in \(\mathbb{R}^2\) for \(\ell_1\), \(\ell_2\), and \(\ell_\infty\) norms. The set \(\{x \in \mathbb{R}^2 : \|x\| \leq 1\}\) for each norm.</div>
</div>
<script>
document.addEventListener('DOMContentLoaded', () => {
  ...
  function legendStyle() {
    return {
      font: {family: figFont, size: 13, color: css('--fig-text')},
      bgcolor: css('--fig-bg') + 'dd',
      bordercolor: css('--fig-grid'),
      borderwidth: 1,
    };
  }
  ...
  function themeLayout(extra = {}) {
    return Object.assign({
      paper_bgcolor: css('--fig-bg'),
      plot_bgcolor: css('--fig-plot-bg'),
      font: { family: figFont, color: css('--fig-text'), size: 14 },
    }, extra);
  }
  ...
  function themeAxis(title, extra = {}) {
    return Object.assign({
      ...
      gridcolor: css('--fig-grid'),
      zerolinecolor: css('--fig-grid'),
      linecolor: css('--fig-axis'),
      tickfont: {family: figFont, size: 12, color: css('--fig-axis')},
    }, extra);
  }

  // l1 ball: diamond |x1| + |x2| <= 1
  const l1x = [1, 0, -1, 0, 1];
  const l1y = [0, 1, 0, -1, 0];

  // l2 ball: circle
  const theta = [];
  for (let i = 0; i <= 100; i++) theta.push(2 * Math.PI * i / 100);
  const l2x = theta.map(t => Math.cos(t));
  const l2y = theta.map(t => Math.sin(t));
  ...
  const traces = [
    { x: linfx, y: linfy, fill: 'toself', fillcolor: css('--fig-lavender'), line: {color: css('--fig-lavender-stroke'), width: 2.5}, name: 'l\u221E ball', mode: 'lines' },
    { x: l2x, y: l2y, fill: 'toself', fillcolor: css('--fig-region'), line: {color: css('--fig-region-stroke'), width: 2.5}, name: 'l\u2082 ball', mode: 'lines' },
    { x: l1x, y: l1y, fill: 'toself', fillcolor: css('--fig-sage'), line: {color: css('--fig-sage-stroke'), width: 2.5}, name: 'l\u2081 ball', mode: 'lines' },
  ];
  const layout = themeLayout({
    title: {text: 'Unit Norm Balls in R\u00B2', font: {family: figFontHeading, size: 16, color: css('--fig-text')}},
    xaxis: themeAxis('x\u2081', {range: [-1.5, 1.5], zeroline: true, zerolinecolor: css('--fig-axis')}),
    yaxis: themeAxis('x\u2082', {range: [-1.5, 1.5], zeroline: true, zerolinecolor: css('--fig-axis'), scaleanchor: 'x'}),
    showlegend: true, legend: Object.assign({x: 0.7, y: 1}, legendStyle()),
    margin: {t: 48, b: 50, l: 55, r: 20},
  });
  Plotly.newPlot('fig-norm-balls', traces, layout, plotlyConfig);
});
</script>
```
...
::: {#thm-norm-inequalities}
## Theorem 2.1 (Norm Inequalities)
...
::: {.callout-tip}
## Remark: Triangle Inequality for $\ell_2$-norm
...
### Equivalence of $\ell_p$-Norms in $\mathbb{R}^n$ {#sec-norm-equivalence}
...
## Theorem 2.2 (Equivalence of $\ell_p$-Norms)

For any $p, q \in [1, +\infty)$, there exist constants $c_{p,q}$ and $C_{p,q}$ such that
...
## Inner Products & Cauchy--Schwarz Inequality {#sec-inner-products}
...
### Inner Product Definition {#sec-inner-product-def}
...
::: {#def-inner-product}
## Definition 2.2 (Inner Product)

An **inner product** $\langle \cdot, \cdot \rangle : \mathcal{X} \times \mathcal{X} \to \mathbb{R}$ defined on a general space $\mathcal{X}$ satisfies the following properties:
...
### Cauchy--Schwarz Inequality {#sec-cauchy-schwarz}

::: {#thm-cauchy-schwarz}
## Theorem 2.3 (Cauchy--Schwarz Inequality)

For any inner product and its induced norm, we always have:
$$\langle x, y \rangle \;\leq\; \|x\| \cdot \|y\| \;=\; \sqrt{\langle x, x \rangle} \cdot \sqrt{\langle y, y \rangle}.$$ {#eq-cauchy-schwarz}
:::

The Cauchy--Schwarz inequality (@eq-cauchy-schwarz) is one of the most fundamental results in all of mathematics. It says that the inner product of two vectors is bounded by the product of their norms, with equality if and only if the vectors are parallel. It underlies the triangle inequality for inner-product-induced norms and appears repeatedly in optimization convergence proofs.
...
::: {#def-matrix-inner-product}
## Definition 2.3 (Matrix Inner Product)
...
### Frobenius Norm {#sec-frobenius-norm}

::: {#def-frobenius-norm}
## Definition 2.4 (Frobenius Norm)

The **Frobenius norm** $\|\cdot\|_F : \mathbb{R}^{m \times n} \to \mathbb{R}$ is the norm induced by the matrix inner product:
...
### Operator / Induced Norms {#sec-operator-norms}
...
::: {#def-operator-norms}
## Definition 2.5 (Operator Norms / Induced Norms)

For any $1 \leq p \leq \infty$, the matrix norm induced by $\ell_p$-vector norms is:
...
::: {.callout-tip}
## Remark: Meaning of Operator Norms
...
```{=html}
<div class="figure-container">
  <div id="fig-operator-norm"></div>
  <div class="figure-caption">Figure 2.2: The unit \(\ell_2\)-ball (left) mapped by \(A = \begin{pmatrix} 1 & 2 \\ 0 & 2 \end{pmatrix}\) to an ellipse (right). The operator norm \(\|A\|_2\) equals the largest semi-axis length.</div>
</div>
<script>
document.addEventListener('DOMContentLoaded', () => {
  const plotlyConfig = {responsive: true, displayModeBar: false};
  ...
  // A = [[1,2],[0,2]] maps circle to ellipse
  const A = [[1,2],[0,2]];
  const ex = theta.map(t => A[0][0]*Math.cos(t) + A[0][1]*Math.sin(t));
  const ey = theta.map(t => A[1][0]*Math.cos(t) + A[1][1]*Math.sin(t));

  const traces = [
    { x: cx, y: cy, mode: 'lines', line: {color: css('--fig-region-stroke'), width: 2.5}, name: 'Unit l\u2082 ball', xaxis: 'x', yaxis: 'y' },
    { x: ex, y: ey, fill: 'toself', fillcolor: css('--fig-sage'), mode: 'lines', line: {color: css('--fig-sage-stroke'), width: 2.5}, name: 'Image A{l\u2082 ball}', xaxis: 'x2', yaxis: 'y2' },
    { x: [0], y: [0], mode: 'markers', marker: {color: css('--fig-vertex'), size: 6}, showlegend: false, xaxis: 'x2', yaxis: 'y2' },
  ];
  const layout = themeLayout({
    grid: {rows: 1, columns: 2, pattern: 'independent'},
    xaxis: themeAxis('Unit Ball', {range: [-2, 2], domain: [0, 0.42]}),
    yaxis: themeAxis('', {range: [-3.5, 3.5], scaleanchor: 'x'}),
    xaxis2: themeAxis('Image under A', {range: [-4, 4], domain: [0.55, 1]}),
    yaxis2: themeAxis('', {range: [-3.5, 3.5], scaleanchor: 'x2', anchor: 'x2'}),
    showlegend: true, legend: legendStyle(),
    margin: {t: 20, b: 55, l: 45, r: 20},
    annotations: [{
      x: 0.23, y: 0.5, xref: 'paper', yref: 'paper',
      text: 'A \u2192', showarrow: false,
      font: {family: figFontHeading, size: 20, color: css('--fig-text')}
    }]
  });
  Plotly.newPlot('fig-operator-norm', traces, layout, plotlyConfig);
});
</script>
```

::: {#thm-induced-norm-properties}
## Theorem 2.4 (Properties of Induced Norms)

- **Consistent:** $\|Ax\|_p \leq \|A\|_p \cdot \|x\|_p$ for all $x \in \mathbb{R}^n$.
- **Submultiplicative:** $\|AB\|_p \leq \|A\|_p \cdot \|B\|_p$.

All induced norms are submultiplicative. The Frobenius norm is also submultiplicative. Not every matrix norm is submultiplicative (e.g., $\|A\|_{\max}$).
:::

These properties are essential for bounding errors in iterative algorithms. The consistency property $\|Ax\|_p \leq \|A\|_p \cdot \|x\|_p$ lets us control how much a linear map can amplify a vector, while submultiplicativity lets us bound the norm of a product of matrices by the product of their norms.

::: {#thm-closed-form-induced-norms}
...
::: {.proof}
**Proof of (i): $\|A\|_1$ = max column sum.**
...
For $A = \begin{pmatrix} 1 & 2 \\ 0 & 2 \end{pmatrix}$:
...
### Nuclear Norm {#sec-nuclear-norm}

::: {#def-nuclear-norm}
## Definition 2.6 (Nuclear Norm)

The **nuclear norm** (also denoted $\|\cdot\|_*$ or $\|\cdot\|_{\mathrm{nuc}}$) is the sum of singular values:
...
## Eigenvalue and Singular Value Decompositions {#sec-eigenvalue-svd}
...
### Eigendecomposition {#sec-eigendecomp}

::: {#thm-eigendecomposition}
## Theorem 2.6 (Eigendecomposition of Symmetric Matrices)

Let $A \in \mathbb{R}^{n \times n}$ be a **symmetric** matrix, i.e., $A^\top = A$. Then $A$ admits an eigendecomposition:
$$A = \sum_{i=1}^{n} \lambda_i \cdot v_i v_i^\top = V \Lambda V^\top$$
where:

- $V = (v_1, v_2, \ldots, v_n)$ is an **orthogonal matrix** ($VV^\top = V^\top V = I_n$).
- $\Lambda = \operatorname{diag}(\lambda_1, \lambda_2, \ldots, \lambda_n)$ with $\lambda_1 \geq \lambda_2 \geq \cdots \geq \lambda_n$.
:::
...
Let $A \in \mathbb{R}^{m \times n}$ be a matrix with rank $k$. Then we can write:
$$A = \sum_{i=1}^{k} \sigma_i \cdot u_i v_i^\top = U \Sigma V^\top$$
where:

- $U = (u_1, u_2, \ldots, u_m) \in \mathbb{R}^{m \times m}$ is an orthogonal matrix.
- $V = (v_1, \ldots, v_n) \in \mathbb{R}^{n \times n}$ is an orthogonal matrix.
- $\Sigma = \operatorname{diag}(\sigma_1, \sigma_2, \ldots, \sigma_k, 0, \ldots, 0)$ with $\sigma_1 \geq \sigma_2 \geq \cdots \geq \sigma_k > 0$.

Consequently, $A^\top A = V \Sigma^2 V^\top$ and $AA^\top = U \Sigma^2 U^\top$, with eigenvalues $\sigma_i^2$.
:::

The SVD generalizes eigendecomposition to arbitrary (non-square, non-symmetric) matrices. It decomposes any matrix into a sum of rank-one terms $\sigma_i u_i v_i^\top$, ordered by decreasing importance. The singular values $\sigma_i$ measure the "importance" of each component, and truncating the sum after the first few terms gives the best low-rank approximation to $A$ (the Eckart--Young theorem).

```{=html}
<div class="figure-container">
  <div id="fig-eigenvalues"></div>
  <div class="figure-caption">Figure 2.3: Eigenvalue spectrum of a symmetric matrix. The eigenvalues determine the shape of the associated quadratic form \(x^\top A x\).</div>
</div>
<script>
document.addEventListener('DOMContentLoaded', () => {
  const plotlyConfig = {responsive: true, displayModeBar: false};
  ...
  for (let i = 0; i <= N; i++) { xvals.push(-2 + 4 * i / N); yvals.push(-2 + 4 * i / N); }

  // A1: positive definite [[4,2],[2,3]] -> eigenvalues ~1.38, 5.62
  const z1 = yvals.map(y => xvals.map(x => 4*x*x + 4*x*y + 3*y*y));
  // A2: indefinite [[2,0],[0,-2]] -> eigenvalues 2, -2
  const z2 = yvals.map(y => xvals.map(x => 2*x*x - 2*y*y));

  const traces = [
    { z: z1, x: xvals, y: yvals, type: 'contour',
      colorscale: [[0, css('--fig-region-stroke')], [0.5, css('--fig-plot-bg')], [1, css('--fig-contour')]],
      contours: {coloring: 'lines', showlabels: true, labelfont: {family: figFont, size: 10, color: css('--fig-text')}},
      ncontours: 12, showscale: false, name: 'x\u1D40Ax (PD)', xaxis: 'x', yaxis: 'y'
    },
    { z: z2, x: xvals, y: yvals, type: 'contour',
      colorscale: [[0, css('--fig-optimal')], [0.5, css('--fig-plot-bg')], [1, css('--fig-sage-stroke')]],
      contours: {coloring: 'lines', showlabels: true, labelfont: {family: figFont, size: 10, color: css('--fig-text')}},
      ncontours: 12, showscale: false, name: 'x\u1D40Bx (Indef.)', xaxis: 'x2', yaxis: 'y2'
    },
  ];
  const layout = themeLayout({
    grid: {rows: 1, columns: 2, pattern: 'independent'},
    xaxis: themeAxis('PD: \u03BB\u2081=5.62, \u03BB\u2082=1.38', {range: [-2, 2], domain: [0, 0.45]}),
    yaxis: themeAxis('', {range: [-2, 2], scaleanchor: 'x'}),
    xaxis2: themeAxis('Indefinite: \u03BB\u2081=2, \u03BB\u2082=\u22122', {range: [-2, 2], domain: [0.55, 1]}),
    yaxis2: themeAxis('', {range: [-2, 2], scaleanchor: 'x2', anchor: 'x2'}),
    showlegend: false,
    margin: {t: 20, b: 55, l: 45, r: 20},
  });
  Plotly.newPlot('fig-eigenvalues', traces, layout, plotlyConfig);
});
</script>
```

### Power Iteration {#sec-power-iteration}
...
::: {.callout-tip}
## Remark: Power Iteration Edge Cases
...
## Multivariate Calculus: Gradient and Hessian {#sec-calculus}
...
::: {#def-gradient}
## Definition 2.7 (Partial Derivatives and Gradient)

The $i$-th **partial derivative** $\frac{\partial f}{\partial x_i} : \mathbb{R}^n \to \mathbb{R}$ is defined by only perturbing the $i$-th entry at $a$:
...
::: {#thm-linear-approximation}
## Theorem 2.8 (Linear Approximation via Gradient)

Let $\delta \in \mathbb{R}^n$ be a small perturbation. Then:
$$f(a + \delta) \approx f(a) + \sum_{i=1}^{n} \delta_i \cdot \frac{\partial f}{\partial x_i}(a) = f(a) + \langle \nabla f(a), \delta \rangle.$$ {#eq-linear-approximation}
...
::: {#def-hessian}
## Definition 2.8 (Hessian)

The **Hessian** of $f$ at point $x \in \mathbb{R}^n$ is defined as:
...
$f^1(x,y) = x^2 + y^2 \;\Rightarrow\; \nabla^2 f^1(x,y) = \begin{pmatrix} 2 & 0 \\ 0 & 2 \end{pmatrix}$ (positive definite --- bowl shape).

$f^2(x,y) = x^2 - y^2 \;\Rightarrow\; \nabla^2 f^2(x,y) = \begin{pmatrix} 2 & 0 \\ 0 & -2 \end{pmatrix}$ (indefinite --- saddle shape).
:::

::: {#thm-quadratic-approximation}
## Theorem 2.9 (Quadratic Approximation via Hessian)

The Hessian gives a **quadratic approximation** around $a$:
$$f(x) \approx f(a) + \langle \nabla f(a), x - a \rangle + \tfrac{1}{2}(x - a)^\top \nabla^2 f(a)\, (x - a).$$ {#eq-quadratic-approximation}
:::

This quadratic approximation (@eq-quadratic-approximation) extends the linear approximation (@eq-linear-approximation) by incorporating curvature via the Hessian. While the gradient provides a tangent plane approximation, the Hessian adds second-order information, yielding a much more accurate local model of $f$. Newton's method exploits this by minimizing the quadratic model at each step.

## Chain Rule and Backpropagation {#sec-chain-rule}

### Chain Rule {#sec-chain-rule-def}
...
Let $f_1, \ldots, f_m : \mathbb{R}^n \to \mathbb{R}$ be $m$ functions, and let $g : \mathbb{R}^m \to \mathbb{R}$. Consider the composite function:
...
Let $h(x_1, x_2) = \cos(x_1) + \exp(x_2) \cdot x_1$. Then:
...
### Automatic Differentiation: Backpropagation {#sec-autodiff}

This systematic application of the chain rule is exactly **backpropagation** --- automatic differentiation via **computational graphs** and the chain rule.

::: {#def-computational-graph}
## Definition 2.9 (Computational Graph)

A **computational graph** $\mathcal{G} = (\mathcal{V}, \mathcal{E})$ is a directed graph which encodes a computation relationship:

- **Node set** $\mathcal{V} = \{v_1, \ldots, v_N\}$: quantities to be computed (input, intermediate values, output).
- Each **edge** $e = (v_i \to v_j) \in \mathcal{E}$ means $v_j$ uses $v_i$ as input.
:::

A computational graph makes the dependency structure of a calculation explicit. Every modern deep learning framework (PyTorch, TensorFlow, JAX) constructs such a graph automatically, enabling efficient gradient computation via automatic differentiation.

Input $x$ goes through a computational graph, intermediate variables are computed, and the final output is the loss $J$. The derivative $\frac{\partial J}{\partial x_i}$ can be calculated by going **backwards** on the computational graph using the chain rule.

::: {.callout-important}
## Algorithm: Forward Mode AD

To compute $\frac{\partial f}{\partial x_i}$: compute $\frac{\partial v}{\partial x_i}$ for all nodes $v$ in $\mathcal{V}$ using a **forward pass**.

If $v_i = h_i(\{v_j : (j \to i) \in \mathcal{E}\})$, then:
$$\frac{\partial v_i}{\partial x_\ell} = \sum_{j:\, (j \to i) \in \mathcal{E}} \frac{\partial h_i}{\partial v_j} \cdot \frac{\partial v_j}{\partial x_\ell}.$$

Cost: one forward pass per input dimension. Slow when $\dim(\text{input})$ is large.
:::

::: {.callout-important}
## Algorithm: Reverse Mode AD (Backpropagation)

Deep learning packages implement **Reverse Mode AD**.

Define the **adjoint**: $\bar{v}_i = \frac{\partial y}{\partial v_i}$ (assuming a single output $y$).

**Idea:** For any $v_j \in \mathcal{V}$ such that $(j \to i) \in \mathcal{E}$, $\bar{v}_j$ receives a contribution $\frac{\partial v_i}{\partial v_j} \cdot \bar{v}_i$ from $v_i$. Thus:
$$\bar{v}_j = \sum_{i:\, (j \to i) \in \mathcal{E}} \frac{\partial v_i}{\partial v_j} \cdot \bar{v}_i.$$

**Steps:**

```
1. Run a forward pass to evaluate all nodes {v_j}_{j in V}.
2. Compute partial derivatives {dv_j/dv_i : (j -> i) in E}.  // new edge weights
3. Evaluate each v_bar_j reversely on the graph.
```

Cost: **one forward + one backward pass**, regardless of input dimension!
:::

::: {.callout-note appearance="simple"}
**Main takeaway:** Whenever we can build a computational graph, we know how to compute the gradient. This is the general philosophy of deep learning.
:::

![Computation graph for backpropagation. Forward arrows (solid, green) propagate activations from input $x$ through layers $W_1$, ReLU, $W_2$ to the loss $\mathcal{L}$. Backward arrows (dashed, red) propagate gradients via the chain rule.](figures/ch03-backpropagation.svg){#fig-backpropagation}
