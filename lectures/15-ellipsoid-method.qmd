...
![Ellipsoids in 2D with different shape matrices $Q$. Left: a ball ($Q = I$). Center: an axis-aligned ellipse. Right: a rotated ellipse. The eigenvectors of $Q$ determine the principal axes.](figures/ellipsoid-shapes.svg){#fig-ellipsoid-shapes}
...
![The ellipsoid method for feasibility. The center $c$ is outside $P$, so a violated constraint provides a separating hyperplane. $P$ lies entirely in the lower half-ellipsoid. The new (green) ellipsoid covers this half and is strictly smaller.](figures/ellipsoid-cut.svg){#fig-ellipsoid-cut}
...
![The MVEE update. Given a half-ellipsoid (shaded), the minimum volume enclosing ellipsoid (green dashed) is computed using the update formulas. The new center shifts away from the cut.](figures/mvee-update.svg){#fig-mvee-update}
...
![Volume shrinkage of the ellipsoid over iterations for different dimensions $n$. The volume decreases exponentially, with a per-step shrinkage factor of $\exp(-1/(2(n+1)))$. Higher dimensions lead to slower per-step shrinkage.](figures/volume-shrinkage.svg){#fig-volume-shrinkage}
...
![The two scenarios of the ellipsoid method for convex optimization. Left (Scenario 1): the center is feasible but not optimal --- the gradient provides the cutting plane. Right (Scenario 2): the center is infeasible --- the separation oracle provides the cutting plane.](figures/ellipsoid-scenarios.svg){#fig-ellipsoid-scenarios}
