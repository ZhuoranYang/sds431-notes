...
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ZhuoranYang/zhuoran-yale-teaching/blob/main/SDS431/notebooks/23-equality-constrained-newton.ipynb) **Equality-Constrained Newton** --- implementing the equality-constrained Newton method step by step
:::
...
::: {#thm-equality-kkt}
## Theorem 19.1 (KKT Conditions for Equality-Constrained Problems)
...
## Theorem 19.2 (Properties of Primal-Dual Newton)
...
```python
#| label: fig-feasible-vs-infeasible
#| fig-cap: "Figure 19.1: Feasible-start Newton (blue) stays on the constraint line, while infeasible-start Newton (rose) approaches the constraint and then follows it. Contours show the objective function."
#| echo: false

import plotly.graph_objects as go
import numpy as np

# Objective: f(x,y) = (x-3)^2 + 4*(y-2)^2
# Constraint: x + y = 3  =>  y = 3 - x

x_grid = np.linspace(-0.5, 5.5, 200)
y_grid = np.linspace(-0.5, 5.5, 200)
X, Y = np.meshgrid(x_grid, y_grid)
Z = (X - 3)**2 + 4*(Y - 2)**2

# Constraint line
cx = np.linspace(-0.5, 5.5, 100)
cy = 3 - cx

# Feasible start iterates (starting on constraint, converging to optimum on constraint)
fs_x = [0.5, 1.2, 1.65, 1.78, 1.8]
fs_y = [2.5, 1.8, 1.35, 1.22, 1.2]

# Infeasible start iterates (starting off constraint, then jumping to it)
inf_x = [1.0, 1.8, 1.82, 1.8]
inf_y = [3.5, 1.6, 1.2, 1.2]

fig = go.Figure()

fig.add_trace(go.Contour(
    x=x_grid, y=y_grid, z=Z,
    colorscale=[[0, 'rgba(184,153,94,0.1)'], [0.5, 'rgba(184,153,94,0.3)'], [1, 'rgba(184,153,94,0.6)']],
    contours=dict(start=0.5, end=20, size=2),
    showscale=False, name='Objective contours', hoverinfo='skip'
))

fig.add_trace(go.Scatter(
    x=cx, y=cy, mode='lines',
    line=dict(color='#968a80', width=2.5, dash='dash'),
    name='Constraint: x + y = 3'
))

fig.add_trace(go.Scatter(
    x=fs_x, y=fs_y, mode='lines+markers',
    line=dict(color='#7b97ad', width=2.5),
    marker=dict(color='#7b97ad', size=9, line=dict(width=1, color='white')),
    name='Version I (feasible start)'
))

fig.add_trace(go.Scatter(
    x=inf_x, y=inf_y, mode='lines+markers',
    line=dict(color='#b56b6b', width=2.5),
    marker=dict(color='#b56b6b', size=9, symbol='diamond',
    line=dict(width=1, color='white')),
    name='Version II (infeasible start)'
))

fig.add_trace(go.Scatter(
    x=[1.8], y=[1.2], mode='markers',
    marker=dict(color='#b8995e', size=14, symbol='star',
    line=dict(width=1.5, color='white')),
    name='Optimal x*'
))

fig.update_layout(
    xaxis=dict(title='x₁', range=[-0.5, 5.5], zeroline=True),
    yaxis=dict(title='x₂', range=[-0.5, 5.5], zeroline=True, scaleanchor='x'),
...
## Definition 19.2 (Constrained Newton Decrement)
...
::: {#thm-decrement-gap}
## Theorem 19.3 (Interpretation of the Newton Decrement)
...
## Definition 19.3 (Logarithmic Barrier Approximation)
...
```python
#| label: fig-barrier-approximation
#| fig-cap: "Figure 19.2: The log barrier approximation −(1/t)log(−u) for several values of t. As t increases, the barrier more closely approximates the indicator function I(u) = 0 for u ≤ 0 and +∞ for u > 0."
#| echo: false

import plotly.graph_objects as go
import numpy as np

u = np.linspace(-2, -0.01, 500)

colors = ['#b8995e', '#b56b6b', '#7b97ad', '#7a9a7e', '#907ea0']
t_vals = [0.5, 1, 2, 5, 10]
...
fig.add_trace(go.Scatter(
    x=[-2, 0, 0], y=[0, 0, None],
    mode='lines', line=dict(color='#968a80', width=3, dash='dot'),
    name='Indicator I(u)'
))
fig.add_trace(go.Scatter(
    x=[0, 0], y=[0, 4],
...
for t_val, color in zip(t_vals, colors):
    barrier = -(1.0/t_val) * np.log(-u)
    mask = barrier < 4.5
    fig.add_trace(go.Scatter(
        x=u[mask], y=barrier[mask], mode='lines',
        line=dict(color=color, width=2.2),
        name=f't = {t_val}'
    ))

fig.update_layout(
    xaxis=dict(title='u', range=[-2.1, 0.3], zeroline=True),
    yaxis=dict(title='Barrier value', range=[-1.5, 4.5], zeroline=True),
    showlegend=True, margin=dict(t=48, b=50, l=55, r=20)
)
fig.show()
```
...
::: {#def-log-barrier}
## Definition 19.4 (Logarithmic Barrier Function)
...
![The logarithmic barrier function. As the barrier parameter $t$ increases, $-(1/t)\log(-u)$ more closely approximates the indicator function $I_-(u)$ that is zero for $u < 0$ and $+\infty$ for $u \geq 0$. The barrier creates a smooth "wall" that goes to $+\infty$ as $x$ approaches the constraint boundary, keeping iterates strictly in the interior of the feasible set.](figures/ch04-log-barrier.svg){#fig-log-barrier}
...
::: {#def-barrier-problem}
## Definition 19.5 (Barrier Problem)
...
## Theorem 19.4 (Properties of the Central Path)
...
```python
#| label: fig-lp-central-path
#| fig-cap: "Figure 19.3: The central path for a 2D LP. The path begins at the analytic center (t ≈ 0) and curves toward the optimal vertex as t → ∞. Each dot represents x*(t) for a particular value of t."
#| echo: false

import plotly.graph_objects as go
import numpy as np

# Define a 2D polytope: pentagon
# Constraints: a_i^T x <= b_i
# x1 >= 0, x2 >= 0, x1 + 2*x2 <= 8, 2*x1 + x2 <= 8, x1 + x2 <= 5.5
A_ineq = np.array([[-1, 0], [0, -1], [1, 2], [2, 1], [1, 1]])
b_ineq = np.array([0, 0, 8, 8, 5.5])
c_obj = np.array([-2, -1])  # min c^T x  => optimal near vertex with large x1

# Vertices of the polytope (computed by hand / intersection)
verts = np.array([
    [0, 0], [4, 0], [3.333, 1.333], [1.0, 3.5], [0, 4]
])
# Close polygon
poly_x = list(verts[:, 0]) + [verts[0, 0]]
poly_y = list(verts[:, 1]) + [verts[0, 1]]

# Central path: solve t*c + nabla phi(x) = 0 numerically
from scipy.optimize import minimize

def barrier_obj(x_flat, t_val, c_vec, A_mat, b_vec):
    x = np.array(x_flat)
    slacks = b_vec - A_mat @ x
    if np.any(slacks <= 1e-12):
        return 1e12
    return t_val * c_vec @ x - np.sum(np.log(slacks))

central_path_pts = []
t_values = [0.01, 0.1, 0.5, 1, 2, 5, 10, 30, 80, 200]
x_init = np.array([1.5, 1.5])

for t_val in t_values:
    res = minimize(barrier_obj, x_init, args=(t_val, c_obj, A_ineq, b_ineq),
                   method='Nelder-Mead', options={'xatol': 1e-10, 'fatol': 1e-10, 'maxiter': 10000})
    central_path_pts.append(res.x.copy())
    x_init = res.x.copy()

cp = np.array(central_path_pts)

# Objective contours
x_grid = np.linspace(-0.5, 5.5, 200)
y_grid = np.linspace(-0.5, 5.5, 200)
X, Y = np.meshgrid(x_grid, y_grid)
Z_obj = c_obj[0] * X + c_obj[1] * Y

fig = go.Figure()

# Contours
fig.add_trace(go.Contour(
    x=x_grid, y=y_grid, z=Z_obj,
    colorscale=[[0, 'rgba(184,153,94,0.08)'], [0.5, 'rgba(184,153,94,0.2)'], [1, 'rgba(184,153,94,0.4)']],
    contours=dict(start=-10, end=2, size=1),
    showscale=False, name='Objective contours', hoverinfo='skip'
))
...
fig.add_trace(go.Scatter(
    ...
    fillcolor='rgba(123,154,126,0.15)', line=dict(color='#7a9a7e', width=2.5),
    name='Feasible polytope', mode='lines'
))

# Central path
fig.add_trace(go.Scatter(
    x=cp[:, 0], y=cp[:, 1], mode='lines',
    line=dict(color='#907ea0', width=3),
    name='Central path'
))

fig.add_trace(go.Scatter(
    x=cp[:, 0], y=cp[:, 1], mode='markers',
    marker=dict(color='#907ea0', size=8, line=dict(width=1, color='white')),
    text=[f't = {t_values[i]}' for i in range(len(t_values))],
    hoverinfo='text', name='x*(t)'
))

# Analytic center (approx t=0)
fig.add_trace(go.Scatter(
    x=[cp[0, 0]], y=[cp[0, 1]], mode='markers',
    marker=dict(color='#b8995e', size=12, symbol='diamond',
    line=dict(width=1.5, color='white')),
    name='Analytic center (t ≈ 0)'
))

# Optimal vertex
fig.add_trace(go.Scatter(
    x=[4.0], y=[0.0], mode='markers',
    marker=dict(color='#b56b6b', size=14, symbol='star',
    line=dict(width=1.5, color='white')),
    name='Optimal vertex x*'
))
...
::: {#thm-perturbed-kkt}
## Theorem 19.5 (Perturbed KKT Conditions)
...
::: {#def-barrier-method}
## Definition 19.7 (Barrier Method)
...
## Theorem 19.6 (Convergence of the Barrier Method)
...
::: {.proof}
**Proof.** After $k$ outer iterations, $t^k = \mu^k t^0$. By the duality gap bound (Theorem 19.4, Property 5), $f(x^k) - p^* \leq m/t^k = m/(\mu^k t^0)$. Setting $m/(\mu^k t^0) \leq \varepsilon$ and solving for $k$ gives the result. $\blacksquare$
:::
...
#| fig-cap: "Figure 19.4: Convergence of the barrier method. Left: duality gap m/t decreases geometrically with each outer iteration. Right: effect of the growth factor μ on the total number of Newton steps."
#| echo: false
...
fig = make_subplots(rows=1, cols=2,
    subplot_titles=('Duality gap vs. outer iteration', 'Total Newton steps vs. μ'))

# Left: gap convergence for different mu
m_constraints = 10
t0 = 1.0
mu_vals = [2, 5, 10, 50]
colors_left = ['#7a9a7e', '#7b97ad', '#b8995e', '#b56b6b']

for mu, color in zip(mu_vals, colors_left):
    iters = np.arange(0, 30)
    gap = m_constraints / (t0 * mu**iters)
    mask = gap > 1e-8
    fig.add_trace(go.Scatter(
        x=iters[mask], y=gap[mask], mode='lines+markers',
        line=dict(color=color, width=2),
        marker=dict(size=5, color=color),
        name=f'μ = {mu}', legendgroup=f'mu{mu}'
    ), row=1, col=1)

# Right: total Newton steps vs mu (schematic)
mu_range = np.linspace(1.1, 100, 200)
# Outer iters = log(m/(eps*t0)) / log(mu)
# Newton per centering ~ c * (mu - 1) for large mu
eps = 1e-4
outer = np.log(m_constraints / (eps * t0)) / np.log(mu_range)
# Approx Newton steps per centering (increases with mu)
newton_per = 3 + 0.8 * (mu_range - 1)
total = outer * newton_per

fig.add_trace(go.Scatter(
    x=mu_range, y=total, mode='lines',
    line=dict(color='#907ea0', width=2.5),
    name='Total Newton steps', showlegend=True
), row=1, col=2)

# Mark optimal mu region
opt_idx = np.argmin(total)
fig.add_trace(go.Scatter(
    x=[mu_range[opt_idx]], y=[total[opt_idx]], mode='markers',
    marker=dict(color='#b56b6b', size=12, symbol='star',
    line=dict(width=1.5, color='white')),
    name=f'Optimal μ ≈ {mu_range[opt_idx]:.1f}', showlegend=True
), row=1, col=2)

fig.update_xaxes(title_text='Outer iteration k', row=1, col=1)
fig.update_yaxes(title_text='Duality gap m/t', type='log', row=1, col=1)
fig.update_xaxes(title_text='Growth factor μ', row=1, col=2)
fig.update_yaxes(title_text='Total Newton steps', row=1, col=2)
...
Interior point methods are the theoretical backbone of modern optimization solvers:
