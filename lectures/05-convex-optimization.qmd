...
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ZhuoranYang/sds431-notes/blob/main/notebooks/05a-convex-optimization-examples.ipynb) **Convex Optimization Examples** --- LP, QP, SDP, robust optimization, and Chebyshev center in CVXPY
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ZhuoranYang/sds431-notes/blob/main/notebooks/05b-portfolio-optimization.ipynb) **Portfolio Optimization** --- Markowitz mean-variance optimization with leverage constraints
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ZhuoranYang/sds431-notes/blob/main/notebooks/05c-control-cvxpy.ipynb) **Optimal Control** --- control trajectory optimization with state and input constraints
- [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ZhuoranYang/sds431-notes/blob/main/notebooks/05d-logistic-regression.ipynb) **Logistic Regression** --- $\ell_1$-regularized logistic regression with cross-validation
:::
...
## General Optimization --- Standard Form
...
## Domain, Feasible Set, Optimal Value
...
::: {#def-optimal-point}
## Optimal Point, Active Constraint, Local Optimality
...
The following examples illustrate the range of possible behaviors for unconstrained optimization in one dimension (@fig-unconstrained-examples):
...
![Four behaviors of unconstrained optimization: (a) infimum not attained, (b) unbounded below, (c) unique global minimum, (d) local minimum that is not global.](figures/ch05-unconstrained-examples.svg){#fig-unconstrained-examples}
...
A **convex optimization problem** is a problem in standard form with three additional requirements:
...
Each sublevel set $\{x : f_i(x) \leq 0\}$ is convex (by @thm-sublevel-sets-convex), each hyperplane is convex, and the intersection of convex sets is convex (by @thm-operations-convex-sets).
...
## Convex $\varepsilon$-Suboptimal Sets
...
::: {.proof}
Let $S_\varepsilon = \{x \in \Omega : f_0(x) \leq f^* + \varepsilon\}$. Take any $x, y \in S_\varepsilon$ and $\lambda \in [0,1]$. Since $\Omega$ is convex, $z = \lambda x + (1{-}\lambda)y \in \Omega$. By convexity of $f_0$,
...
```{=html}
<div id="fig-local-global" style="width:100%;max-width:800px;margin:auto;"></div>
<p class="figure-caption" style="text-align:center;margin-top:0.5em;">Figure 5.1: A convex function (left) has a single global minimum. A non-convex function (right) can have multiple local minima that are not global.</p>
<script>
(function() {
  var x = Array.from({length: 200}, function(_, i) { return -3 + 6 * i / 199; });
  var convexY = x.map(function(v) { return v * v; });
  var nonconvexY = x.map(function(v) { return Math.pow(v, 4) - 3 * v * v + 1; });

  var traces = [
    {x: x, y: convexY, mode: 'lines', line: {color: '#7b97ad', width: 3}, showlegend: false, xaxis: 'x', yaxis: 'y'},
    {x: [0], y: [0], mode: 'markers', marker: {color: '#b56b6b', size: 12, symbol: 'star'}, name: 'Global min', xaxis: 'x', yaxis: 'y'},
    {x: x, y: nonconvexY, mode: 'lines', line: {color: '#ad8480', width: 3}, showlegend: false, xaxis: 'x2', yaxis: 'y2'},
    {x: [-1.22, 1.22], y: [-1.25, -1.25], mode: 'markers', marker: {color: '#b56b6b', size: 12, symbol: 'star'}, name: 'Global minima', xaxis: 'x2', yaxis: 'y2'},
    {x: [0], y: [1], mode: 'markers', marker: {color: '#b8995e', size: 10, symbol: 'diamond'}, name: 'Local max', xaxis: 'x2', yaxis: 'y2'}
  ];

  var layout = {
    height: 400,
    margin: {t: 40, b: 50, l: 55, r: 20},
    xaxis: {title: {text: 'x'}, domain: [0, 0.45]},
    yaxis: {title: {text: 'f(x)'}, anchor: 'x'},
    xaxis2: {title: {text: 'x'}, domain: [0.55, 1]},
    yaxis2: {title: {text: 'f(x)'}, anchor: 'x2'},
    annotations: [
      {text: 'Convex: f(x) = x\u00B2', font: {size: 14}, showarrow: false, x: 0.225, xref: 'paper', y: 1.02, yref: 'paper', xanchor: 'center'},
      {text: 'Non-convex: x\u2074 \u2212 3x\u00B2 + 1', font: {size: 14}, showarrow: false, x: 0.775, xref: 'paper', y: 1.02, yref: 'paper', xanchor: 'center'}
    ]
  };

  Plotly.newPlot('fig-local-global', traces, layout, {responsive: true, displayModeBar: false});
})();
</script>
```
...
For unconstrained convex problems, the familiar calculus condition $\nabla f_0(x^*) = 0$ is both necessary and sufficient for global optimality --- a direct consequence of the first-order characterization of convexity (@eq-first-order-convexity).
...
::: {#thm-equality-constrained-optimality}
## Equality-Constrained Optimality

Consider $\min f_0(x)$ subject to $Ax = b$, where $f_0$ is convex and differentiable. Then $x^*$ is optimal if and only if:

$$
\nabla f_0(x^*) \perp \mathcal{N}(A),
$$ {#eq-equality-optimality}

where $\mathcal{N}(A) = \{v : Av = 0\}$ is the null space of $A$.
:::

::: {.proof}
The feasible set is $\Omega = \{x : Ax = b\}$. By the general constrained optimality condition (@thm-first-order-constrained below), $x^*$ is optimal if and only if $\nabla f_0(x^*)^\top(y - x^*) \geq 0$ for all feasible $y$. Since both $x^*$ and $y$ satisfy $Ax = b$, we have $A(y - x^*) = 0$, so $y - x^*$ ranges over the null space $\mathcal{N}(A)$. The condition becomes $\nabla f_0(x^*)^\top v \geq 0$ for all $v \in \mathcal{N}(A)$. Since $\mathcal{N}(A)$ is a subspace (if $v \in \mathcal{N}(A)$ then $-v \in \mathcal{N}(A)$), this forces $\nabla f_0(x^*)^\top v = 0$ for all $v \in \mathcal{N}(A)$. $\blacksquare$
:::

Geometrically, @eq-equality-optimality says that at the optimum, the gradient of the objective has no component along the affine subspace of feasible directions --- any movement that stays feasible cannot decrease $f_0$.
...
![First-order optimality for constrained convex problems. At the optimal point $x^*$ on the boundary of the feasible set $\Omega$, the negative gradient $-\nabla f_0(x^*)$ points away from $\Omega$, so $\langle \nabla f_0(x^*), y - x^* \rangle \geq 0$ for all feasible $y \in \Omega$ --- no feasible descent direction exists.](figures/ch05-optimality-condition.svg){#fig-constrained-optimality}
...
The constrained optimality condition @eq-constrained-optimality generalizes both the unconstrained case (@eq-unconstrained-optimality, where $\Omega = \mathbb{R}^n$ and the condition reduces to $\nabla f_0(x^*) = 0$) and the equality-constrained case (@eq-equality-optimality, where feasible directions are restricted to $\mathcal{N}(A)$).
...
Given $m$ producers with supply limits $s_i$ and $n$ consumers with demands $d_j$, and per-unit transport cost $c_{ij}$ from producer $i$ to consumer $j$ (@fig-transportation):
...
![The transportation problem as a bipartite graph. Each producer $P_i$ (with supply $s_i$) can ship to each consumer $D_j$ (with demand $d_j$) at unit cost $c_{ij}$. The decision variables $x_{ij} \geq 0$ represent the flow on each route.](figures/ch05-transportation.svg){#fig-transportation}

::: {#exm-max-flow}
## Maximum Flow Problem

Given a directed graph $G = (V, E)$ with edge capacities $C_{uv}$, maximize flow from source to sink (@fig-max-flow):
...
![A maximum flow instance on a 4-node network. Each edge is labeled flow/capacity. The total flow from source $s$ to sink $t$ is 4.](figures/ch05-max-flow.svg){#fig-max-flow}

::: {#exm-svm-lp}
## Linear Classification (Support Vector Machine)

Given $m$ data points $\{(x_i, y_i)\}_{i=1}^m$ with $x_i \in \mathbb{R}^n$ and labels $y_i \in \{+1, -1\}$, the data is **linearly separable** if there exists a hyperplane $w^\top x + b = 0$ that separates the two classes. Finding such a hyperplane amounts to finding $(w, b)$ with $y_i(w^\top x_i + b) \geq 1$ for all $i$, which is a **feasibility LP**.

Among all separating hyperplanes, the **max-margin classifier** chooses the one that maximizes the distance (margin $= 2/\|w\|_2$) between the closest points of each class and the hyperplane (@fig-svm-margin):
...
![Max-margin SVM. The separating hyperplane $w^\top x = b$ (solid) maximizes the margin $2/\|w\|$ between the two classes. Support vectors (large markers) lie exactly on the margin boundaries $w^\top x = b \pm 1$ (dashed). The normal vector $w$ points toward the positive class.](figures/ch05-svm-margin.svg){#fig-svm-margin}
...
![LP vs QP: both have the same polyhedral feasible set $Ax \leq b$. **Left:** LP with linear contours --- the optimum is always at a vertex. **Right:** QP with elliptical contours --- the optimum can lie on a face or in the interior, not necessarily at a vertex.](figures/ch05-lp-vs-qp.svg){#fig-lp-vs-qp}

::: {#exm-least-squares-constraints}
## Least Squares with Constraints

The constrained least-squares problem $\min \|Ax - b\|_2^2 \;\text{s.t.}\; x \geq 0$ is a QP with $P = 2A^\top A$, $q = -2A^\top b$. Similarly, the Lasso $\min \|y - X\beta\|_2^2$ subject to $\|\beta\|_1 \leq s$ is a QP (after reformulating the $\ell_1$-constraint using auxiliary variables).
:::

![Nonnegative least squares. The elliptical contours show level sets of $\|Ax - b\|^2$. The unconstrained minimizer $x_{\text{LS}}$ (terracotta) is infeasible; the constrained optimum $x^*$ (sage) is the point in the first quadrant closest to $x_{\text{LS}}$ in objective value.](figures/ch05-constrained-ls.svg){#fig-constrained-ls}

::: {#exm-portfolio}
## Markowitz Portfolio Optimization

Given $n$ assets with expected return vector $\mu \in \mathbb{R}^n$ and covariance matrix $\Sigma \in \mathbb{S}^n_+$, an investor allocates weight $x_i$ to asset $i$ (with $\sum_i x_i = 1$, $x_i \geq 0$). The portfolio variance is $x^\top \Sigma x$ and the expected return is $\mu^\top x$ (@fig-portfolio-frontier). The **minimum-variance portfolio** for a target return $r_{\min}$ is:
...
![Markowitz efficient frontier. The bullet-shaped region shows all achievable (risk, return) pairs. The **efficient frontier** (sage, thick) traces portfolios that maximize return for each risk level. The minimum-variance portfolio (gold) is the leftmost point. For a target return $r_{\min}$ (dashed), the optimal portfolio minimizes risk on the frontier.](figures/ch05-portfolio-frontier.svg){#fig-portfolio-frontier}

::: {#exm-distance-polyhedra}
## Distance Between Polyhedra

Given two polyhedra $P_1 = \{x : A_1 x \leq b_1\}$ and $P_2 = \{x : A_2 x \leq b_2\}$ (@fig-distance-polyhedra), their distance $\operatorname{dist}(P_1, P_2) = \inf_{x_1 \in P_1, x_2 \in P_2} \|x_1 - x_2\|_2$ can be computed by solving the QP:
...
![Distance between two polyhedra. The closest points $x_1^* \in P_1$ and $x_2^* \in P_2$ achieve the minimum $\|x_1 - x_2\|_2$ over all pairs, giving a QP with linear constraints.](figures/ch05-distance-polyhedra.svg){#fig-distance-polyhedra}

### Quadratically Constrained QP (QCQP) {#sec-qcqp}
...
A **QCQP** minimizes a convex quadratic objective subject to convex quadratic inequality constraints:
...
At each step of a trust-region method, we minimize a local quadratic model $m(s) = f(x) + \nabla f(x)^\top s + \tfrac{1}{2}s^\top \nabla^2 f(x)\, s$ within a trust region $\|s\| \leq \Delta$ (@fig-trust-region):
...
![Trust region optimization. The quadratic model $m(s)$ (dashed contours) has its unconstrained minimizer outside the trust region $\|s\| \leq \Delta$ (sage circle). The trust-region solution $s^*$ lies on the boundary, in the direction of the unconstrained minimizer.](figures/ch05-trust-region.svg){#fig-trust-region}

::: {#exm-portfolio-risk}
## Portfolio Optimization with Risk Constraints

Suppose asset returns have a factor structure: the risk exposure is captured by a matrix $F \in \mathbb{R}^{k \times n}$ (where $F_{ij}$ is asset $j$'s exposure to risk factor $i$). We want to minimize variance while bounding the total factor risk exposure $\|Fw\|_2^2 \leq \text{risk}_{\max}$:
...
In many applications, the constraint coefficients $a_i$ are not known exactly but lie in an ellipsoidal uncertainty set $\mathcal{E}_i = \{\bar{a}_i + P_i u : \|u\|_2 \leq 1\}$ (where $P_i \in \mathbb{R}^{n \times n}$). A **robust LP** requires the constraint to hold for *all* possible values of $a_i$ (@fig-robust-lp):
...
![Robust LP with ellipsoidal uncertainty. Each nominal constraint (blue, solid) has an associated uncertainty set $\mathcal{U}_i$ (gold ellipses). The robust feasible set (green, solid) is the intersection of all worst-case constraints --- it shrinks inward from the nominal feasible set (blue, dashed) by the amount $\|P_i^\top x\|_2$.](figures/ch05-robust-lp.svg){#fig-robust-lp}

::: {#exm-smallest-enclosing-circle}
## Smallest Enclosing Circle

Given $n$ points $(x_i, y_i)$ in $\mathbb{R}^2$, find the smallest circle enclosing all of them (@fig-enclosing-circle). With center $(c_x, c_y)$ and radius $R$:
...
![Smallest enclosing circle. The optimal circle (sage) of radius $R$ centered at $(c_x, c_y)$ encloses all data points (blue). Three points lie exactly on the boundary --- they determine the solution.](figures/ch05-enclosing-circle.svg){#fig-enclosing-circle}

::: {#exm-robust-classifier}
## Robust Classification and Max-Margin SVM

Suppose each data point $x_i$ is not known exactly but lies in a ball $\mathcal{E}_i = \{x_i + u : \|u\|_2 \leq \rho\}$. We want a linear classifier $(w, b)$ that correctly classifies *every* point in every ball: $y_i(w^\top(x_i + u) + b) \geq 1$ for all $\|u\| \leq \rho$. Taking the worst case over $u$ gives $y_i(w^\top x_i + b) - \rho\|w\|_2 \geq 1$, or equivalently:
...
A **semidefinite program** (in standard form) optimizes a linear objective over the cone of positive semidefinite matrices:
...
::: {#exm-eigenvalue-sdp}
## Computing the Largest Eigenvalue via SDP

The largest eigenvalue of a symmetric matrix $\Sigma$ can be expressed as $\lambda_{\max}(\Sigma) = \max_{\|x\|_2 = 1} x^\top \Sigma x$. This is a nonconvex problem (the constraint $\|x\| = 1$ is not convex). However, it admits an exact **SDP relaxation**. Writing $X = xx^\top$, the problem becomes $\max \langle \Sigma, X \rangle$ subject to $\langle I, X \rangle = 1$, $X = xx^\top$. Relaxing the rank-one constraint $X = xx^\top$ to $X \succeq 0$ gives the SDP:
...
<div class="figure-caption" style="text-align:center; color:#3d3530; font-size:0.92em; margin-top:0.3em;">Figure 5.2: Nested hierarchy of convex programs. Each inner class is a special case of the outer classes. Moving outward increases modeling power but generally increases computational cost. LP is solvable in polynomial time; QP and SOCP admit efficient interior-point methods; SDP is the most general class with polynomial-time algorithms.</div>
</div>
```
...
Given a polytope $P = \{x : a_i^\top x + b_i \geq 0\}$, find the largest circle (center $x_c$, radius $r$) inside $P$ (@fig-inscribed-circle):
...
![Largest inscribed circle (Chebyshev center) in a polytope. The circle of radius $r$ centered at $x_c$ is tangent to the nearest faces. The distance from $x_c$ to face $i$ is $(a_i^\top x_c + b_i)/\|a_i\| \geq r$, giving an LP in $(x_c, r)$.](figures/ch05-inscribed-circle.svg){#fig-inscribed-circle}
...
### Eliminating and Introducing Equality Constraints {#sec-eliminate-equality}
...
### Minimizing Over Some Variables {#sec-partial-min}
...
where $\widetilde{f}_0(x_1) = \inf_{x_2 : g_j(x_2) \leq 0} f_0(x_1, x_2)$ is convex by the partial minimization rule (@thm-operations-convex-functions).

::: {#exm-schur-complement}
## Schur Complement

Consider minimizing a block-quadratic over $x_2$:
...
## Solving Convex Programs with CVXPY {#sec-cvxpy}
...
[**CVXPY**](https://www.cvxpy.org/) is a Python library that implements **disciplined convex programming** (DCP): a system of composition rules that can verify convexity of an optimization problem at specification time, before any numerical computation begins. The DCP rules mirror the preservation rules from @thm-operations-convex-functions --- nonnegative sums of convex functions, compositions with monotone functions, pointwise maxima, etc. If CVXPY can verify that a problem follows these rules, it knows the problem is convex and can solve it.
...
The Markowitz portfolio problem from @exm-portfolio can be solved in just a few lines:
...
```{=html}
<div id="fig-efficient-frontier" style="width:100%;max-width:700px;margin:auto;"></div>
<p class="figure-caption" style="text-align:center;margin-top:0.5em;">Figure: Markowitz efficient frontier for a two-asset portfolio. The frontier traces the minimum-variance portfolio for each target return. No portfolio below the frontier is achievable; portfolios above it are suboptimal.</p>
<script>
(function() {
  // Two-asset efficient frontier (analytical)
  var mu1 = 0.05, mu2 = 0.14, s1 = 0.10, s2 = 0.28, rho = 0.25;
  var N = 200;
  var risks = [], rets = [];
  for (var i = 0; i < N; i++) {
    var w = -0.2 + 1.4 * i / (N - 1);  // weight on asset 1
    var r = w * mu1 + (1 - w) * mu2;
    var v = w*w*s1*s1 + (1-w)*(1-w)*s2*s2 + 2*w*(1-w)*rho*s1*s2;
    risks.push(Math.sqrt(v));
    rets.push(r);
  }
  // Minimum variance portfolio
  var wMin = (s2*s2 - rho*s1*s2) / (s1*s1 + s2*s2 - 2*rho*s1*s2);
  var rMin = wMin*mu1 + (1-wMin)*mu2;
  var vMin = wMin*wMin*s1*s1 + (1-wMin)*(1-wMin)*s2*s2 + 2*wMin*(1-wMin)*rho*s1*s2;

  var traces = [
    {x: risks, y: rets, mode: 'lines', line: {color: '#7b97ad', width: 3}, name: 'Efficient frontier'},
    {x: [s1, s2], y: [mu1, mu2], mode: 'markers+text', marker: {color: '#c47a6a', size: 12, symbol: 'diamond'},
     text: ['Bonds', 'Stocks'], textposition: ['bottom center', 'top center'],
     textfont: {size: 12, color: '#3d3530'}, name: 'Individual assets', showlegend: true},
    {x: [Math.sqrt(vMin)], y: [rMin], mode: 'markers', marker: {color: '#7a9a7e', size: 14, symbol: 'star'},
     name: 'Min-variance portfolio'}
  ];

  var layout = {
    height: 450, margin: {t: 20, b: 55, l: 65, r: 20},
    xaxis: {title: {text: 'Portfolio risk (std dev)', font: {size: 14}}, range: [0.05, 0.32],
            gridcolor: '#e0d9d1', linecolor: '#968a80'},
    yaxis: {title: {text: 'Expected return', font: {size: 14}}, range: [0.02, 0.16],
            gridcolor: '#e0d9d1', linecolor: '#968a80', tickformat: '.0%'},
    legend: {x: 0.02, y: 0.98, font: {size: 12}},
    paper_bgcolor: 'rgba(0,0,0,0)', plot_bgcolor: 'rgba(0,0,0,0)',
    font: {family: '"Source Serif 4", Georgia, serif'}
  };

  Plotly.newPlot('fig-efficient-frontier', traces, layout, {responsive: true, displayModeBar: false});
})();
</script>
```

### Example: Chebyshev Center of a Polytope

The largest inscribed circle problem from @exm-inscribed-circle finds the **Chebyshev center** of a polytope --- the point farthest from all boundary faces:
...
```{=html}
<div id="fig-chebyshev" style="width:100%;max-width:600px;margin:auto;"></div>
<p class="figure-caption" style="text-align:center;margin-top:0.5em;">Figure: Chebyshev center (largest inscribed circle) of a polytope. The center maximizes the minimum distance to all boundary faces.</p>
<script>
(function() {
  // Polytope defined by Ax <= b with A = [[1,0],[-1,0],[0,1],[0,-1],[1,1],[-1,-1]], b = [2,2,2,2,3,3]
  // Vertices (computed from pairwise active constraints, ordered counterclockwise):
  // (2,1), (1,2), (-2,2), (-2,-1), (-1,-2), (2,-2)
  var polyX = [2, 1, -2, -2, -1, 2, 2];
  var polyY = [1, 2, 2, -1, -2, -2, 1];

  // Chebyshev center and radius: by symmetry center = (0,0), min distance to faces = 2
  var cx = 0, cy = 0, rad = 2.0;

  // Circle
  var cTheta = Array.from({length: 100}, function(_, i) { return 2*Math.PI*i/99; });
  var circX = cTheta.map(function(t) { return cx + rad * Math.cos(t); });
  var circY = cTheta.map(function(t) { return cy + rad * Math.sin(t); });

  var traces = [
    {x: polyX, y: polyY, fill: 'toself', fillcolor: 'rgba(123,151,173,0.12)',
     line: {color: '#7b97ad', width: 2.5}, name: 'Polytope', mode: 'lines'},
    {x: circX, y: circY, mode: 'lines', line: {color: '#c47a6a', width: 2.5, dash: 'dash'},
     fill: 'toself', fillcolor: 'rgba(196,122,106,0.1)', name: 'Inscribed circle'},
    {x: [cx], y: [cy], mode: 'markers', marker: {color: '#c47a6a', size: 10, symbol: 'x'},
     name: 'Chebyshev center'}
  ];

  var layout = {
    height: 450, margin: {t: 20, b: 50, l: 55, r: 20},
    xaxis: {title: {text: 'x₁'}, range: [-3, 3], scaleanchor: 'y', gridcolor: '#e0d9d1', linecolor: '#968a80'},
    yaxis: {title: {text: 'x₂'}, range: [-3, 3], gridcolor: '#e0d9d1', linecolor: '#968a80'},
    legend: {x: 0.02, y: 0.98, font: {size: 12}},
    paper_bgcolor: 'rgba(0,0,0,0)', plot_bgcolor: 'rgba(0,0,0,0)',
    font: {family: '"Source Serif 4", Georgia, serif'}
  };
...
```{=html}
<div id="fig-lasso-path" style="width:100%;max-width:700px;margin:auto;"></div>
<p class="figure-caption" style="text-align:center;margin-top:0.5em;">Figure: Lasso regularization path. Each curve shows how one coefficient evolves as the penalty λ increases. Coefficients shrink toward zero; for the orthogonal case shown, the soft-thresholding formula β*(λ) = sign(β̂)·max(|β̂| - λ, 0) applies.</p>
<script>
(function() {
  // Simulate orthogonal-design Lasso path (soft thresholding)
  var betaOLS = [3.0, -2.0, 0.3, -0.1, 1.5, 0.2, -0.15, -1.0];
  var colors = ['#7b97ad', '#c47a6a', '#7a9a7e', '#b8995e', '#907ea0', '#ad8480', '#968a80', '#3d3530'];
  var N = 200;
  var lambdas = Array.from({length: N}, function(_, i) { return Math.pow(10, -2 + 4*i/(N-1)); });

  var traces = betaOLS.map(function(b, idx) {
    var vals = lambdas.map(function(lam) {
      var absBeta = Math.abs(b);
      return Math.sign(b) * Math.max(absBeta - lam, 0);
    });
    return {
      x: lambdas, y: vals, mode: 'lines', line: {color: colors[idx], width: 2},
      name: 'β' + (idx+1)
    };
  });

  // Zero line
  traces.push({x: [0.01, 100], y: [0, 0], mode: 'lines',
    line: {color: '#968a80', width: 0.8, dash: 'dot'}, showlegend: false});

  var layout = {
    height: 400, margin: {t: 20, b: 55, l: 65, r: 20},
    xaxis: {title: {text: 'λ (regularization strength)', font: {size: 14}}, type: 'log',
            gridcolor: '#e0d9d1', linecolor: '#968a80'},
    yaxis: {title: {text: 'Coefficient value', font: {size: 14}}, range: [-2.5, 3.5],
            gridcolor: '#e0d9d1', linecolor: '#968a80'},
  ...
  Plotly.newPlot('fig-lasso-path', traces, layout, {responsive: true, displayModeBar: false});
})();
</script>
```
